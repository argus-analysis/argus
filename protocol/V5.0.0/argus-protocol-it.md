---
title: "ARGUS V5.0.0 — Protocollo completo"
lang: it
...
description: "Protocollo completo ARGUS V5.0.0"
...
- punti di convergenza, passo per passo, una riga ciascuno;
...
- un caso particolare da segnalare sistematicamente: divergenze in cui un'analisi conclude e l'altra dichiara elementi insufficienti (cfr. Passo 7). Isolano il punto esatto in cui una delle due è andata oltre ciò che il testo consente di stabilire;
- divergenze di copertura: constatazioni sollevate da una sola delle due analisi e non menzionate dall'altra. Trattale separatamente dalle divergenze di posizione. Per ciascuna, l'analisi che non l'ha sollevata deve dichiarare se la respinge dopo averla esaminata, se non l'ha esaminata o se considera che vada oltre ciò che il testo consente. Una constatazione non menzionata non è né concessa né contestata, e la regola di aggiudicazione non si applica prima di questa risposta;
...
- un segno che mostra che il contenuto effettivamente fornito è interrotto da un paywall, come «restano X da leggere»; la sola menzione «articolo riservato agli abbonati» non indica una troncatura, poiché compare anche nelle copie complete di articoli a pagamento;
- un testo che si interrompe a metà di una frase, di un paragrafo o di un'enumerazione;
- un programma annunciato nell'apertura le cui parti corrispondenti sono assenti;
- un rinvio interno a uno sviluppo che non compare nel documento;
- un riferimento a una tabella, un grafico, una nota o un allegato assente, quando la tesi ne dipende;
- un indicatore di serie i cui altri elementi mancano, come «2 di 4», se il testo fornito presuppone i precedenti;
- un sommario introduttivo che annuncia una dimostrazione di cui il corpo non contiene alcun elemento.
...
- il Passo 5 non può concludere che vi sia uno scarto tra il programma annunciato e il contenuto effettivo, poiché il contenuto mancante può adempiere l'impegno; lo scarto osservato viene allora dichiarato non conclusivo;
- il test delle assenze di 3.E non può qualificare come omissione un elemento che la parte mancante potrebbe contenere; le assenze identificate vengono dichiarate con riserva;
- il livello di 7.c, in linea di principio, non è assegnabile allo stato attuale, ai sensi della clausola del Passo 7, salvo che gli elementi presenti siano sufficienti da soli a stabilirlo, cosa che deve essere giustificata.
...
- **Pertinenza parziale**: il testo presenta una dimensione argomentativa, ma il protocollo dovrà essere adattato o limitato a determinati passaggi. Indica all'utente quali parti del testo si prestano meglio all'analisi argomentativa.
I criteri si valutano sull'intero testo presentato, non sulla sua parte più argomentativa. Quando sono soddisfatti soltanto in parti identificabili, la pertinenza è parziale per composizione, anche se tali parti soddisfano tutti i criteri: prevale la composizione dell'oggetto, non la forza della sua parte argomentativa. La pertinenza forte presuppone quindi un testo argomentativo nel suo insieme. Quando due qualificazioni restano entrambe difendibili per lo stesso oggetto, conserva la pertinenza parziale per composizione, che lascia aperta l'analisi della parte argomentativa.
...
- **Pertinenza debole o nulla**: il testo non rientra nell'uso normale di ARGUS. Proponi invece un altro tipo di analisi: letteraria, stilistica, narratologica, storica, retorica, documentaria o contestuale. Se l'utente richiede espressamente l'analisi nonostante questo giudizio, essa viene prodotta e inizia con la menzione «pertinenza debole, analisi richiesta dall'utente». Non si tratta di una quarta uscita dall'interruzione, ma di una deroga dichiarata, e si applica tanto a un testo isolato quanto a un testo di corpus.
...
- la qualificazione di pertinenza, con la sua forma e la sua ragione in una riga;
- il perimetro proposto, con le sezioni incluse ed escluse nominate;
- il regime secondo il quale verrebbero esaminate le sezioni escluse, quando applicabile.
...
- **Perimetro già fissato dall'utente nella richiesta**: l'analisi lo adotta senza chiedere una nuova validazione e dichiara in testa: «perimetro fissato dall'utente nella richiesta; nessuna validazione aggiuntiva necessaria». Questo perimetro è considerato validato ai fini di 0.a bis; non viene presentato come una decisione unilaterale dell'analista.
- **Validazione esplicitamente dispensata senza che l'utente abbia fissato un perimetro**: l'analista fissa autonomamente il perimetro e dichiara in testa: «perimetro non validato dall'utente, fissato unilateralmente dall'analista». Non è una formalità: indica al lettore che la decisione più decisiva dell'analisi non ha ricevuto un controllo esterno.
...
- **Parziale per grado**: il testo è debolmente argomentativo nel suo insieme.
- **Parziale per composizione**: il testo è pienamente argomentativo in alcune parti e didattico, tecnico, documentario o narrativo in altre. È il caso della maggior parte degli oggetti reali: un rapporto che contiene una tesi e un allegato di dati, un dossier stampa che combina editoriale e reportage, un libro in cui un capitolo argomenta e diversi insegnano.
...
- **Editoriale / articolo d'opinione**: espressione di un'opinione, intento persuasivo. Le esigenze di prova fattuale sono basse; l'analisi si concentrerà principalmente su logica, coerenza e retorica.
- **Reportage / articolo di stampa**: informazione fattuale, inchiesta, testimonianza. I fatti centrali (date, luoghi, cifre, citazioni) devono essere documentati o attribuiti. Le generalizzazioni sono tollerate se restano ragionevoli.
...
- **Articolo scientifico / tecnico**: esigenza massima di prove, dati e metodi riproducibili.
- **Argomentazione di parte / manifesto**: difesa esplicita di una causa, parzialità dichiarata. L'analisi si concentrerà sulla coerenza dell'argomentazione e sulla trasparenza del punto di vista.
- **Testo militante di schieramento**: mobilitazione di un pubblico già affine, spesso mediante un vocabolario combattivo. L'analisi si concentrerà sul grado di chiusura interpretativa.
- **Propaganda**: testo la cui organizzazione protegge la conclusione dall'esame autonomo del lettore mediante una combinazione di selezione dei fatti, neutralizzazione del controargomento, asimmetrie, omissioni o meccanismi di autoimmunizzazione. La diagnosi e il suo grado vengono stabiliti in 7.c: quel passo distingue la propaganda di Livello 3 dalla propaganda bloccata di Livello 4 e tratta i modi frontale o sofisticato come descrittori ortogonali al livello. Questa classificazione in 0.b descrive soltanto il contratto di lettura installato dal testo; non è una diagnosi. In caso di dubbio al Passo 0.b, conserva una designazione descrittiva e riserva la diagnosi a 7.c.
- **Altro** (specificare).
...
- **Fonte A**: nome, funzione, affiliazione istituzionale, paese, posizione dichiarata sul tema, eventuali finanziamenti o legami con parti interessate.
- **Fonte B**: idem.
...
- **Pubblico reale probabile**: il pubblico effettivamente preso di mira, dati il media, il supporto, il registro linguistico, i riferimenti culturali, il livello di tecnicità, la sezione, il modo di diffusione e il contesto di pubblicazione.
- **Pubblico strategico**: il pubblico che il testo cerca soprattutto di convincere, rafforzare, mobilitare, rassicurare o immunizzare contro obiezioni.
- **Pubblico respinto**: il pubblico o schieramento che il testo non cerca di convincere, ma di squalificare, ridicolizzare o escludere dal perimetro del dicibile.

Rispondi poi alle domande seguenti:

- **Quali codici di credibilità si aspetta questo pubblico?**
  Esempi: competenza, citazioni, cifre, riferimenti storici, moderazione apparente, ironia, indignazione morale, testimonianza diretta, semplicità, complessità, tecnicità, neutralità del tono.

- **Quali segni farebbero apparire il testo poco credibile o troppo propagandistico agli occhi di questo pubblico?**
  Esempi: assenza totale di controargomento, pathos eccessivo, semplificazione troppo visibile, slogan, vocabolario militante, fonti eccessivamente omogenee, accusa troppo diretta.
...
- **Quali obiezioni il pubblico target sarebbe più incline a formulare?**
...
- **Il testo è calibrato per produrre un effetto di serietà presso questo pubblico specifico?**
  Se sì, quali dispositivi svolgono questa funzione?
...
- **Test di definizione**: il testo fornisce, per questo termine, una definizione, un criterio di applicazione o un'enumerazione di ciò che comprende? Si tratta di una constatazione di presenza o assenza, senza valutare la qualità della definizione. Una caratterizzazione evocativa che non fornisce alcun criterio per sapere se il termine si applica a un caso determinato non soddisfa il test: il test è allora positivo, nonostante l'abbondanza di commenti che il testo dedica al termine. È il caso più difficile da vedere, perché il testo appare esplicito su una parola di cui non fissa mai il confine.
- **Test di confine**: nomina due possibili delimitazioni del termine, una restrittiva e una estensiva, entrambe compatibili con il modo in cui il testo lo usa. Riformula la conclusione sotto ciascuna. Se la conclusione regge sotto una delimitazione e fallisce sotto l'altra, in qualunque direzione, il confine del termine sostiene l'argomentazione e il test è positivo. Se la conclusione regge sotto entrambe, il termine è vago senza essere strategico e il test è negativo. È il test di sostituzione del Passo 7.a applicato all'estensione di un termine anziché a un elemento di prova: la correzione minima non riguarda un pezzo del dossier ma il confine di una parola.
...
- Vi sono salti inferenziali ingiustificati o slittamenti di significato?
...
- l'esistenza, la denominazione esatta o la natura di un'entità citata come prova contro qualcuno;
- un punto fattuale isolato da cui dipende un'inferenza e che una sola ricerca basterebbe a stabilire.

Ogni verifica esterna è accompagnata dalle condizioni seguenti:

- notificare esplicitamente che l'analisi esce dal perimetro stretto del testo;
- assicurarsi che la verifica riguardi l'oggetto dichiarato in testa all'analisi, non un'altra versione o edizione dello stesso testo;
- citare la fonte esterna (documento pubblico, studio, dichiarazione contraria, ecc.);
- riconoscere che il testo originale non conteneva questa informazione.
...
- **Esito del controllo**: *riuscito*, documentato secondo la condizione sopra indicata; *parziale*, con le parti stabilite e non stabilite nominate; *infruttuoso*, quando il controllo è stato svolto senza stabilire nessuno degli elementi cercati. Questo esito non presuppone una ricerca online: una fonte fornita che non stabilisce nulla di ciò che si cercava produce un controllo infruttuoso, e i due assi restano indipendenti.
- **Modalità di accesso alla fonte**: *fonte fornita*, confronto con un allegato o documento già presente; *ricerca esterna*, una fonte che è stato necessario cercare.
...
- Le affermazioni fattuali sono supportate all'interno del testo o semplicemente affermate senza prova?
- Gli eventi sono datati con precisione nel testo o la cronologia è vaga, compressa o perfino contraddittoria?
- Se viene stabilita una tipologia o una gerarchia tra fenomeni, ogni termine è descritto accuratamente e completamente nel testo? Fatti o mediazioni che il testo stesso menziona altrove vengono poi omessi o deformati al servizio della tesi?
- Per ogni citazione decisiva: sono indicati opera, data e pagina? Il contesto di enunciazione è quello che il testo analizzato le attribuisce? Una citazione è **ricontestualizzata** quando è autentica ma spostata in una situazione a cui non si riferiva. Questo difetto sopravvive alla verifica letterale, perché le parole sono esatte, e si rileva soltanto risalendo alla fonte.
- **Fedeltà a una fonte primaria disponibile.** Il carattere primario, ufficiale, contemporaneo o direttamente pertinente di un elemento fornito non basta ad attivare questo test. Si attiva soltanto quando il testo analizzato stesso si riferisce a quella fonte — la cita, la nomina, la riassume, la commenta o la critica — oppure quando l'analisi può stabilire che l'affermazione verificata ne riproduce il contenuto. La semplice concordanza di argomento, data, formulazione o cifra non basta a stabilire tale filiazione. In caso contrario, l'elemento resta una verifica esterna ordinaria: può corroborare, contraddire o non corroborare un'affermazione del testo, ma non si applicano le qualificazioni di fedeltà qui sotto. Quando il legame con la fonte è stabilito, le affermazioni decisive che la riguardano devono essere confrontate con essa. Il controllo è circoscritto: riguarda le affermazioni da cui dipendono la tesi del testo o il suo contratto di credibilità, non tutte le frasi. Per ciascuna, qualifica:
  **fedele**, il significato, la portata e il grado di certezza sono preservati;
  **rafforzata**, un'ipotesi, una riserva, una correlazione o una constatazione limitata viene trasformata in un'affermazione più generale o più certa;
  **indebolita**, il testo attenua o restringe ciò che la fonte stabilisce;
  **spostata**, le parole o i dati sono esatti ma attribuiti a un referente, confronto, periodo o scena che la fonte non trattava;
  **inesatta**, la fonte dice altro su un fatto verificabile e decisivo;
  **interpretazione propria segnalata**, il testo aggiunge una lettura che non è quella della fonte e la presenta esplicitamente come propria;
  **non decidibile**, la fonte disponibile non consente di concludere.
  La constatazione «rafforzata» merita particolare attenzione perché può produrre un'affermazione più sicura della sua fonte pur restando letteralmente difendibile. La sua gravità è determinata esclusivamente in 7.a dal test di sostituzione, non dalla sua qualificazione.
  Queste qualificazioni non sono necessariamente esclusive: un'affermazione può essere insieme rafforzata e spostata. Quando si cumulano, nomina la qualificazione principale e quelle secondarie. «Fedele» e «non decidibile», invece, sono incompatibili con qualsiasi qualificazione di difetto relativa allo stesso elemento.
  Tre delle sette qualificazioni non sono difetti e non passano a 7.a: **fedele**, **interpretazione propria segnalata** e **non decidibile**. La prima registra la conformità, la seconda registra che il testo assume una lettura invece di attribuirla alla fonte, e la terza registra un limite dell'analisi e non del testo. Una fonte primaria disponibile non attiva da sola l'Allegato 3 né questo test di fedeltà. Se la fonte primaria con cui il testo si relaziona nelle condizioni sopra indicate non è disponibile, dichiaralo e nomina le affermazioni per le quali resta non decidibile il confine tra ciò che la fonte dice e ciò che il testo aggiunge, il che rientra nella clausola «Quando gli elementi sono insufficienti» del Passo 7.
- Se il testo contiene cifre probatorie ai sensi del §2 dell'Allegato 4, attiva tale allegato e riporta le sue constatazioni in una sottosezione dedicata di 3.B intitolata «Controllo aritmetico e statistico (Allegato 4)».

#### C. Uso di termini simbolicamente carichi

- Identifica le parole che da sole convocano un registro emotivo, culturale o storico massiccio (pane, sangue, terra, il popolo, patria, libertà, ecc.).
- Chiedi se queste parole sostituiscono la dimostrazione con l'evocazione e se la loro forza affettiva immunizza l'argomento contro la contraddizione.
- Questi termini descrivono adeguatamente la realtà o sono inadeguati pur essendo retoricamente potenti? A chi si rivolgono realmente?

#### D. Uso di termini che pretendono universalità

- Quando il testo usa «la specie umana», «planetario», «tutti», «noi», identifica a chi si riferiscono realmente questi termini.
- L'autore parla a nome di un'umanità astratta mentre descrive realtà che riguardano soltanto una parte specifica di tale umanità?
- Il «noi» è una maschera per un pubblico particolare (occidentale, europeo, ecc.) presentato come universale?
...
- Il testo può rendere conto di un fatto che la contraddirebbe o dovrebbe ignorarlo o deformarlo per sopravvivere?

Oltre alla forma della tesi, esamina come il testo la usa.

- Elenca le osservazioni che il testo tratta come conferme.
- Chiedi se un'osservazione di segno opposto viene anch'essa trattata come conferma. In tal caso, qualifica il difetto come **infalsificabilità d'uso**, distinta dall'**immunizzazione formale**: la tesi è confutabile nella formulazione ma assorbe ogni osservazione nell'applicazione. Questa forma di immunizzazione sopravvive a un esame limitato alla formulazione della tesi, poiché tale formulazione è confutabile: il difetto appare soltanto nell'uso.
- Chiedi esplicitamente: quale osservazione il testo riconoscerebbe come confutante? Se non ne nomina nessuna, segnalalo.

#### G. Registro stilistico e leggibilità

- Identifica le frasi la cui complessità sintattica è tale da non poter essere comprese alla prima lettura ed essere incompatibili con l'oralità.
- Chiedi se questa complessità è funzionale (esprime una reale complessità della realtà) o retorica (produce un effetto di autorità intellettuale, filtra il pubblico o maschera un'improprietà logica).
- Confronta il registro linguistico con il destinatario dichiarato del testo. Se il testo afferma di rivolgersi a «tutti», parla un linguaggio realmente accessibile a tutti? Lo scarto tra il «noi» proclamato e il registro effettivo è una contraddizione performativa da segnalare.
...
- Lo stesso difetto può essere riscontrato in chi parla, nel gruppo o nell'istanza che privilegia, oppure nel dispositivo stesso del testo (cornice, omissioni, presupposti)?
- Se sì, il testo lo riconosce? Ne trae le conseguenze?
- Se no, la differenza di trattamento è esplicitamente e validamente giustificata?
...
- **Esiste un'asimmetria sistematica nel trattamento dei diversi attori?** Se sì, il testo la giustifica esplicitamente? Se non fornisce una giustificazione valida, segnala tale asimmetria come debolezza argomentativa.
...
- **Il testo utilizza i marcatori di serietà attesi dal proprio pubblico target?**
  Numerose fonti, esperti, riferimenti storici, tono analitico, prudenza lessicale, cifre, concessioni, complessità sintattica, distinzione apparente tra fatti e interpretazioni.
...
- **Il controargomento è formale o effettivo?**
  Un controargomento è effettivo se obbliga il testo a modificare, limitare o riconfigurare la propria tesi.
  Un controargomento è formale (o decorativo) se viene menzionato ma immediatamente assorbito, marginalizzato o reso innocuo.

- **Le concessioni aprono o immunizzano?**
  Una concessione di apertura rende possibile una conclusione diversa o più incerta.
  Una concessione immunizzante produce un'apparenza di equilibrio proteggendo la conclusione centrale dalla critica.

- **Le omissioni già identificate in 3.E sono precisamente quelle che il pubblico target identificato in 0.f avrebbe bisogno di conoscere per valutare autonomamente la tesi?**

- **Il testo produce chiusura interpretativa sotto forma di complessità?**
  In altre parole: dà al lettore l'impressione di aver attraversato la complessità, mentre gli elementi introdotti non minacciano mai realmente la conclusione?
  In un testo composito, questo test riguarda il perimetro argomentativo dichiarato in 0.a bis, non le sezioni tecniche o didattiche. Una difficoltà tecnica reale che non serve alcuna conclusione non è chiusura interpretativa.

- **Dove si concentra il rigore?**
  Localizza separatamente, nel testo, le affermazioni accompagnate da un riferimento preciso (opera, data, pagina, fonte nominata, metodo dichiarato) e le affermazioni prive di riferimento. Confronta poi le due posizioni. I riferimenti precisi riguardano affermazioni che il pubblico target può verificare autonomamente a basso costo o quelle che sostengono la tesi?
  Un rigore concentrato sul verificabile e assente dal decisivo funziona come dispositivo di credibilità anziché come giustificazione. Produce un effetto di erudizione più ampio del supporto documentario reale, ed è tanto più efficace quanto più impeccabili sono i pochi riferimenti forniti.

Termini raccomandati nell'analisi:

- **Controargomento effettivo**: presenza di un'obiezione realmente presa sul serio.
- **Controargomento decorativo**: presenza di un'obiezione che non modifica la traiettoria argomentativa.
- **Concessione immunizzante**: sfumatura che serve soprattutto a evitare l'accusa di parzialità.
- **Sfumatura bloccata**: sfumatura integrata in modo da rafforzare la conclusione anziché aprirla.
- **Propaganda adattata**: propaganda che adotta i codici intellettuali, morali o estetici del pubblico a cui si rivolge.
- **Rigore spostato**: precisione documentaria concentrata sulle affermazioni più facili da verificare e assente da quelle che sostengono la tesi.
...
- segni di distinzione o posizionamento all'interno di un campo (riferimenti a scuole di pensiero, a «modi sbagliati» di pensare, ad avversari non nominati);
- appelli a un pubblico specifico (segni di complicità, presupposti condivisi, un «noi» il cui perimetro reale deve essere identificato);
- silenzi, omissioni, temi attesi che vengono evitati;
- insistenze, ripetizioni, formulazioni che sembrano mirare a un effetto preciso sul lettore (colpevolizzazione, mobilitazione, legittimazione dell'autore);
- lo scarto tra registro linguistico e destinatario dichiarato;
- tensioni, contraddizioni o ambiguità che suggeriscono che il testo non possa assumere apertamente tutto ciò che fa.
...
- il destinatario reale (a chi parla realmente il testo? Si rivolge ai già convinti per rafforzarli più che agli indecisi per persuaderli? Parla ai propri pari per segnare una posizione in un campo intellettuale più che al grande pubblico per mobilitarlo?);
- la funzione di legittimazione dell'autore (il testo serve a conferire all'autore una posizione di esperto, teorico, figura morale o portavoce?);
- l'effetto ricercato sul lettore (informare, commuovere, colpevolizzare, mobilitare, paralizzare, rafforzare, mettere a tacere) e la sua coerenza con lo scopo dichiarato;
- un'operazione di distinzione o posizionamento all'interno di un campo intellettuale o politico;
- una funzione strategica implicita (per esempio: squalificare un concorrente senza nominarlo, prendere le distanze da una posizione più radicale o più moderata, proteggere una posizione istituzionale, produrre un effetto di legittimazione senza dichiararlo esplicitamente).
...
- **Alto**: l'ipotesi è sostenuta da indizi convergenti e nessun elemento del testo la contraddice.
- **Medio**: esistono indizi, ma sono possibili altre interpretazioni.
- **Basso**: l'ipotesi poggia su indizi tenui o ambigui; è plausibile ma lontana dall'essere certa.
...
- Si astiene da ciò che ha detto che non avrebbe fatto?
- Se viene constatato uno scarto, la dichiarazione iniziale deve essere riqualificata: funziona come dispositivo di autolegittimazione. Lo scarto stabilisce questa funzione; da solo non consente di concludere sulla sincerità dell'autore, e l'analisi non deve farlo.
...
- La cornice iniziale ha predeterminato o protetto l'argomentazione neutralizzando in anticipo determinate obiezioni?
- La tesi sopravvive se si rifiuta il postulato di apertura?
- Gli argomenti sono indipendenti dalla cornice o ne dipendono interamente?
- L'intenzione strategica inferita getta nuova luce sulla funzione del dispositivo di apertura?
...
- **In 7.a**: se la gravità di un difetto non può essere stabilita mediante il test di sostituzione per mancanza di informazioni, il difetto viene elencato con la nota «gravità non determinabile allo stato attuale», anziché essere collocato per impostazione predefinita nella categoria «minore».
- **In 7.b**: se il testo non stabilisce nulla in modo solido, scrivilo. La sezione è obbligatoria; riempirla non lo è. Attribuire al testo qualità che non possiede per rispettare formalmente la sezione sarebbe una compiacenza simmetrica alla severità artificiale.
- **In 7.c**: se gli elementi raccolti in 0.d, 0.e, 0.f, 3.I, 3.J, 3.E e 3.K — i sette passi su cui si costruisce 7.c — non consentono di assegnare un livello, scrivi «livello non assegnabile allo stato attuale», specificando quale elemento manca. Non assegnare per impostazione predefinita il Livello 1.
...
- **Redibitorio**: il difetto invalida la tesi centrale (per esempio: contraddizione logica maggiore, omissione strutturante senza la quale la tesi non regge, falsificazione manifesta).
- **Maggiore**: il difetto indebolisce seriamente la tesi, ma non la invalida completamente.
- **Minore**: il difetto non influisce sulla conclusione complessiva del testo.
...
- Se la conclusione centrale non può più essere mantenuta con la stessa pretesa: **redibitorio**.
- Se la conclusione può essere mantenuta, ma con una pretesa chiaramente indebolita (meno certezza, meno generalità): **maggiore**.
- Se la conclusione e la sua pretesa restano invariate: **minore**.
...
- **Le qualità reali del testo**, come elencate in 7.b, se esistono indipendentemente dai suoi difetti argomentativi. Attenzione: non chiamare «forza retorica» un'efficacia ottenuta mediante fallacie. Non chiamare «coerenza interna» la chiusura circolare di un sistema su sé stesso. Non chiamare «capacità di nominare fenomeni reali» un'evocazione selettiva che omette fatti contrari.

- **I difetti argomentativi precisi**, con il loro livello di gravità (7.a).

- **Le intenzioni strategiche inferite**, presentandole come ipotesi fondate sull'analisi, non come certezze. Ricorda il grado di fiducia associato a ciascuna ipotesi.

- **Articolazione tra intenzione inferita e giudizio complessivo:**
  Se viene riscontrato uno scarto significativo tra scopo dichiarato e scopo inferito, che cosa rivela sulla natura del testo? Un testo può riuscire come atto politico (posizionamento, mobilitazione) e fallire come dimostrazione.
  L'intenzione inferita non confuta la tesi, ma può spiegare le sue debolezze argomentative: un autore che persegue un obiettivo strategico può sacrificare il rigore all'efficacia retorica, non per accidente ma per coerenza con il proprio scopo reale.
  Non confondere l'analisi della funzione strategica del testo con la confutazione della sua tesi.

- **Asimmetrie ingiustificate**: quando applicabile, elenca i difetti che il testo attribuisce al proprio oggetto ma non riconosce in sé stesso o nel proprio schieramento, senza valida giustificazione.

- **Giudizio complessivo**: la tesi è stabilita? Il testo soddisfa la propria pretesa dimostrativa? Se il testo fallisce come dimostrazione ma funziona come manifesto o atto di posizionamento, dichiaralo esplicitamente, senza confondere i due registri di valutazione. Indica se sono stati identificati difetti redibitori.
...
- **Livello 0 — Informazione aperta / non propagandistica**: il testo ha una cornice, come ogni testo, ma nessun bias strutturante protegge la sua conclusione. Le fonti sono sufficientemente diverse per l'oggetto, gli attori comparabili sono trattati simmetricamente e gli elementi contrari possono effettivamente modificare, limitare o rovesciare la conclusione.
- **Livello 1 — Informazione orientata**: una cornice, una selezione o gerarchia dei fatti strutturante, oppure fonti marcatamente unilaterali, orientano la lettura. Il lettore conserva tuttavia gli elementi necessari all'esame autonomo; il controargomento non è sistematicamente neutralizzato e un'osservazione contraria può ancora incidere sulla conclusione. Il testo orienta più di quanto chiuda.
- **Livello 2 — Argomentazione di parte / giornalismo impegnato**: il testo difende attivamente una posizione o conclusione e organizza al suo servizio la selezione, gerarchia o interpretazione dei fatti. Il bias può essere esplicito o inferibile, ma le obiezioni restano capaci di modificare o limitare la conclusione. Il testo cerca di persuadere; non protegge ancora sistematicamente la conclusione dalla contraddizione.
- **Livello 3 — Propaganda**: diversi meccanismi convergenti proteggono la conclusione dall'esame autonomo — forte asimmetria, circolarità informativa, controargomento neutralizzato, concessioni immunizzanti, omissioni strategiche, rigore spostato o altri meccanismi di chiusura. Il lettore può ancora ricostruire una contraddizione, ma il dispositivo lo rende sensibilmente più difficile o la assorbe quando compare. Il termine «propaganda» comincia a questo livello.
- **Livello 4 — Propaganda bloccata**: la protezione diventa autoimmunizzante o materialmente chiusa. Omissioni strutturanti, fabbricazione o manipolazione decisiva, esclusione del controargomento o un meccanismo che trasforma ogni osservazione contraria in conferma impediscono alla contraddizione interna di modificare la conclusione senza un apporto esterno sostanziale. L'esame autonomo non è più soltanto orientato o reso difficile: è impedito sul punto centrale.
...
- Da 0 a 1: appare un orientamento strutturante?
- Da 1 a 2: il testo difende attivamente una conclusione, oltre alla cornice che organizza l'informazione?
- Da 2 a 3: la conclusione è protetta dalla contraddizione mediante meccanismi di neutralizzazione o chiusura?
- Da 3 a 4: questa protezione diventa autoimmunizzante o materialmente tale che la contraddizione non può più produrre il proprio effetto senza un apporto esterno sostanziale?
...
- **Modalità retorica**: *frontale* quando impegno, accusa o mobilitazione sono immediatamente visibili; *sofisticata* quando la persuasione opera principalmente attraverso i codici di credibilità attesi dal pubblico — tecnicità, competenza, moderazione, abbondanza documentaria, concessioni o neutralità del tono.
- **Origine o funzione**: *giornalistica*, *militante*, *aziendale*, *istituzionale*, *statale*, *parastatale* oppure un'altra qualificazione stabilita con precisione. Un'origine statale o istituzionale non implica alcun livello automatico; una comunicazione istituzionale aperta alla contraddizione può essere meno chiusa di un testo privato autoimmunizzante.
...
- Identificazione del pubblico target e del contratto di credibilità (0.f)
- Circolarità informativa (3.I)
- Simmetria nel trattamento degli attori (3.J)
- Presenza e natura delle omissioni strategiche (3.E)
- Funzione effettiva di sfumature, concessioni e moderazioni (3.K)
- Controargomento effettivo o decorativo (3.K)
- Adattamento dei metodi persuasivi alle aspettative del pubblico target (3.K)
...
- Presentare la scala riproducendo tutti e cinque i livelli con le relative etichette.
- Assegnare un livello al perimetro ARGUS del testo analizzato, indicando numero ed etichetta. Per un testo non composito, questo perimetro è l'intero testo.
- Aggiungere, se stabilite e utili, la modalità retorica e l'origine o funzione come descrittori separati dal livello.
- Giustificare l'assegnazione elencando i criteri osservati, con rinvii ai passi corrispondenti (0.d, 0.e, 0.f, 3.I, 3.J, 3.E, 3.K).
- Giustificare esplicitamente la soglia superiore: dire perché il testo supera o non supera il livello successivo. L'esitazione tra due livelli si risolve mediante la domanda corrispondente alla soglia, mai facendo una media.
...
- il livello globale non è la media dei livelli di sottoperimetro; deve essere giustificato come giudizio autonomo, riferito al **perimetro ARGUS considerato nel suo insieme**, e non alle sezioni escluse in 0.a bis;
- ogni livello di sottoperimetro deve essere collegato a sezioni nominate, mai a un passaggio isolato selezionato a posteriori. Senza questa condizione, la sottoassegnazione permetterebbe di estrarre un paragrafo severamente valutato da un testo altrimenti solido, producendo un'immagine non rappresentativa del testo.
...
- **Livello 0 — Informazione aperta / non propagandistica**: cornice senza protezione strutturale della conclusione.
- **Livello 1 — Informazione orientata**: orientamento strutturante, esame autonomo ancora effettivo.
- **Livello 2 — Argomentazione di parte / giornalismo impegnato**: conclusione attivamente difesa, obiezioni ancora capaci di modificarla.
...
**Livello assegnato al perimetro ARGUS analizzato**: Livello [0–4] — [etichetta].
**Modalità retorica, se stabilita**: [frontale / sofisticata].
**Origine o funzione, se stabilita**: [giornalistica / militante / aziendale / istituzionale / statale / parastatale / altra].

**Giustificazione**:

- Diversità delle fonti (0.d): [constatazione].
- Trasparenza sul contesto di produzione (0.e): [constatazione].
- Pubblico target e contratto di credibilità (0.f): [constatazione].
- Circolarità informativa (3.I): [constatazione].
- Simmetria di trattamento (3.J): [constatazione].
- Omissioni strategiche (3.E): [constatazione].
- Controargomento, concessioni e adattamento al pubblico (3.K): [constatazione].
- **Soglia superiore**: [ragione per cui il livello successivo viene o non viene superato].
...
- **Verifiche esterne**: i tre numeri dichiarati coincidono con i controlli riportati nel corpo dell'analisi, ciascuno nel proprio conteggio, e tali controlli riguardano l'oggetto dichiarato anziché un'altra versione del testo?
- **Non contraddizione tra i passi**: un'affermazione del Passo 7 è contraddetta da una constatazione dei Passi 3-6? In particolare, un elemento identificato come punto di rigore del testo può coesistere con una qualificazione che ne presuppone l'assenza?
- **Calcoli**: ogni calcolo riportato in 7.b come esatto e verificato è accompagnato dal valore ricalcolato dall'analista? In caso contrario, viene dichiarato non verificato e non esatto.
- **Constatazioni fattuali**: lo stesso fatto riceve due descrizioni incompatibili in due punti dell'analisi?
- **Fonte primaria**: quando il legame tra il testo e una fonte primaria soddisfaceva le condizioni di attivazione del test di fedeltà di 3.B, le affermazioni decisive sono state confrontate con tale fonte e il risultato qualificato secondo il test? Un elemento primario semplicemente fornito o pertinente è stato impropriamente trattato come fonte del testo senza filiazione stabilita?
- **Qualificazione delle assenze**: a un'assenza è stata negata la qualificazione di omissione strategica con la motivazione che non distruggerebbe la tesi, criterio che appartiene soltanto alla seconda soglia di 3.E? Un'assenza è stata collocata nell'esito «non determinabile allo stato attuale» anche se un controllo ragionevolmente accessibile avrebbe consentito di decidere e non è stato tentato, oppure senza dichiarare l'indisponibilità di qualsiasi controllo? Questo esito vieta entrambe le cose.
- **Registro dell'analisi** (Regola 19): l'analisi contiene elogi o biasimi comparativi che non posso stabilire, oppure un termine carico non richiesto dalla constatazione?
...
- (a) un presupposto non dimostrato ammesso dallo stesso analista;
- (b) un rischio di bias identificato (compreso il bias di serie o di ancoraggio se l'analisi fa parte di una sequenza di testi);
- (c) un limite informativo irrisolto;
- (d) **deferenza**: lo status di una fonte (istituzione riconosciuta, autore canonico, rivista sottoposta a peer review, agenzia di stampa affermata) ha abbassato l'esigenza probatoria dell'analista su un'affermazione precisa? Se sì, quale?
- (e) **conformismo di serie**: se questa analisi segue altre analisi, l'analista ha allineato un giudizio ai precedenti anziché al testo? Su quale punto esatto?
...
- **Presupposti dell'analisi**: quali presupposti non dimostrati ha ammesso lo stesso analista (per esempio, superiorità di un tipo di prova su un altro, legittimità dell'esigenza di fonti esterne, ecc.)?
- **Simmetria epistemica**: l'analista ha applicato al proprio discorso le stesse esigenze di rigore, trasparenza e non contraddizione richieste al testo studiato?
- **Bias potenziali**: l'analista ha ragione di sospettare che le proprie opinioni (politiche, professionali, culturali) abbiano orientato l'analisi?
- **Limiti dell'analisi**: quali informazioni aggiuntive sarebbero necessarie per dirimere determinati punti rimasti indeterminati?
...
15. Regole di formattazione dell'output dell'analisi:
  - Non usare mai tabelle (né Markdown né HTML). Usa elenchi puntati, elenchi numerati, coppie chiave-valore o paragrafi strutturati con sottotitoli.
  - Per gli elenchi puntati, usa esclusivamente il carattere `-` come punto elenco, con una riga vuota prima del primo punto, uno spazio dopo il trattino e nessuna riga vuota tra gli elementi. Sono consentiti anche gli elenchi numerati, secondo lo stesso principio di spaziatura (riga vuota prima, nessuna riga vuota tra gli elementi).
  - Per i sottoelementi, non usare punti elenco secondari. Colloca le informazioni supplementari nelle righe successive, senza punto elenco, con rientro di due spazi.
  - Per le coppie chiave-valore, usa il formato `**Termine**: definizione.`

16. **Non confondere mai il controargomento formale con il controargomento effettivo.** La presenza di una sfumatura, una concessione o una fonte contraria non dimostra l'equilibrio del testo. L'analista deve verificare se tale sfumatura possa realmente modificare la conclusione o serva soltanto a renderla più accettabile per il pubblico target.

17. **Valuta sempre la propaganda in rapporto al suo pubblico target.** Una propaganda rivolta a un pubblico colto può essere documentata, sfumata, prudente e stilisticamente sofisticata. Queste qualità formali non la escludono dal campo della propaganda se servono a chiudere l'interpretazione anziché ad aprirla.

18. **Non produrre mai un'analisi artificialmente completa.** Se il testo non fornisce elementi sufficienti per trattare una sezione in modo affidabile, l'analista deve dichiararlo esplicitamente («Dati insufficienti», «Nessun elemento probatorio», «Questione non decidibile sulla sola base del testo»). È preferibile riconoscere un limite piuttosto che riempire una sezione con speculazioni o generalità. L'applicazione dettagliata di questa regola, con i suoi tre limiti, figura in testa al Passo 7 sotto la clausola «Quando gli elementi sono insufficienti».

19. **Applica i Test 3.C e 3.G alla tua stessa prosa.** Il protocollo esamina il registro del testo analizzato; esamina anche quello dell'analisi. Un'analisi critica che non sorveglia il proprio registro viene contraddetta dalla propria prosa prima di esserlo dalle sue constatazioni. Ne derivano due divieti.
  **Elogio o biasimo comparativo non stabilito.** Accreditare o screditare un testo confrontandolo con una classe di testi («migliore di quanto richieda il genere», «raro in questo tipo di produzione», «come spesso accade in questi ambienti») presuppone un'affermazione empirica su tale classe. Se l'analisi non può stabilirla, il confronto viene eliminato e la constatazione formulata senza termine di paragone. Questa regola è la controparte della Regola 14: quella vieta i moderatori vaghi che attenuano la critica; questa vieta i confronti vaghi che gonfiano un elogio o un rimprovero.
  **Termini carichi nell'analisi.** Il vocabolario identificato da 3.C nel testo analizzato non ha posto nel commento che lo identifica. Test operativo: elimina il termine dispregiativo e verifica che la constatazione regga ancora. Se regge, il termine era superfluo; se non regge, la constatazione non era stabilita.
...
- La norma di prova dichiarata prima dell'esame e indicizzata al genere testuale. Un elenco di fallacie applicato senza tener conto del genere del testo non fornisce questa indicizzazione: tratta allo stesso modo un editoriale e un articolo scientifico, benché i loro contratti di lettura siano diversi.
- La separazione, nel Passo 7.c, tra il **grado di chiusura interpretativa** (Livelli da 0 a 4) e i descrittori di **modalità retorica** (frontale o sofisticata) e di **origine o funzione**. Una propaganda rivolta a un pubblico colto può essere documentata, sfumata e prudente; questi tratti ne descrivono la forma senza determinarne da soli il grado.
- La distinzione tra controargomento formale ed effettivo, e tra concessioni di apertura e immunizzanti.
- La sezione obbligatoria 7.b, che impedisce al protocollo di funzionare come una macchina per concludere contro.
- Il Passo 8, per il quale non ho trovato equivalente negli strumenti consultati.
...
- **Il termine non è mai definito** nel testo, oppure le sue definizioni successive sono incompatibili. Esempio generico: «giustizia», «il popolo», «vera democrazia».
- **Il termine serve a riunire domande eterogenee** senza precisarle. Esempio generico: «vogliamo cambiamento».
- **L'autore oppone due significanti vuoti** per creare una frontiera morale («noi» contro «loro»). Esempio generico: «l'interesse generale» contro «i privilegi».
- **Il termine ha una forte carica emotiva** ma un basso contenuto informativo. Esempio generico: «scandalo», «catastrofe», «emergenza assoluta».
- **L'argomentazione poggia sull'evocazione di questo termine** senza mai aver bisogno di dimostrarlo. Esempio generico: «il momento storico», «la finestra», «lo slancio necessario».

**3. Griglia di analisi di un significante vuoto**

Per ogni termine sospetto, applica sistematicamente:

- **Localizzazione**: Dove compare? In quali momenti strategici (apertura, conclusione, transizione)?
- **Statuto**:
  - **Vuoto**: assenza di definizione, uso intercambiabile.
  - **Pieno**: definito, stabile, non sostituibile senza perdita di significato.
  - **Misto**: vuoto in alcuni punti, pieno in altri (tensione potenziale).
- **Funzione nell'argomento**:
  - Unire (riunire gruppi diversi)
  - Evitare la confutazione (rendere la tesi non falsificabile)
  - Suggerire legittimità senza prova
  - Creare un senso di urgenza
  - Distinguere l'autore da un avversario (respingendo un significante vuoto concorrente)
- **Scarto rispetto a un possibile uso pieno**: Se lo stesso termine è usato come significante pieno altrove (dall'autore o nel campo discorsivo), lo scarto è giustificato? Segnala una contraddizione?
- **Impatto sulla solidità argomentativa**:
  - **Basso**: il vuoto è aneddotico, la tesi regge senza di esso.
  - **Medio**: la tesi dipende in parte da questa vaghezza (per esempio, previsione vaga).
  - **Alto**: senza il significante vuoto, l'argomento crolla (per esempio, «urgenza» senza prova).

**4. Legame con i passi del protocollo ARGUS**

- **Passo 1 – Dispositivo di apertura**: L'apertura può installare uno o più significanti vuoti («crisi», «urgenza», «noi») che incorniciano l'intero testo.
- **Passo 3.C – Termini simbolicamente carichi**: Distingui i termini carichi ma pieni (per esempio, «sciopero») dai termini carichi e vuoti (per esempio, «giustizia»). Questo affina la critica.
- **Passo 3.E – Test delle assenze**: Un significante vuoto può mascherare omissioni: la vaghezza permette di non scegliere tra soluzioni contraddittorie.
- **Passo 3.F – Falsificabilità**: Un significante vuoto rende la tesi difficile da falsificare, perché il contenuto mancante può essere ridefinito a posteriori.
- **Passo 3.H – Simmetria epistemica**: Un testo che usa significanti vuoti per squalificare un avversario usa a sua volta significanti vuoti per legittimarsi?
- **Passo 4 – Intenzione strategica**: L'uso di significanti vuoti può costituire un indizio di un'intenzione di mobilitare più che di dimostrare; il suo peso viene determinato insieme agli altri indizi del Passo 4.
- **Passo 7 – Conclusione differenziata**: Indica se la tesi centrale poggia su uno o più significanti vuoti ingiustificati.
...
- **Termine «crisi»**: non definito (economica? politica? morale?). Intercambiabile con «pericolo», «emergenza». Statuto: **vuoto**. Funzione: creare un senso di urgenza senza prova.
- **Termine «popolo»**: non definito (nazionale? locale? gli oppressi? tutti i cittadini?). Statuto: **vuoto**. Funzione: unire domande eterogenee attorno a un «noi» vago.
- **Termine «finestra»**: nessun contenuto temporale o procedurale. Statuto: **vuoto**. Impatto: **alto** — se si eliminano questi tre significanti vuoti, resta soltanto un'esortazione priva di giustificazione.
...
- L'analista deve annotare, alla fine di ogni analisi individuale, un breve riepilogo dei livelli assegnati in 7.a (gravità) e 7.c (grado di propaganda), per costruire una griglia comparativa in C.3.
- Se l'ordine di analisi non è neutro (per esempio, il corpus è fornito in un ordine cronologico o tematico scelto da un terzo), l'analista deve segnalarlo come possibile fattore di ancoraggio (cfr. C.3).
...
- una cifra, un tasso, una proporzione o una proiezione probatoria nel senso definito qui sotto;
- un confronto quantitativo tra due insiemi;
- un conteggio relativo a un corpus, a una copertura mediatica o a una banca dati documentaria;
- un'affermazione di rilevazione, screening, attribuzione o profilazione;
- una media presentata come caratteristica di una popolazione.
...
- **A. Ordine di grandezza.** Estrapola il tasso indicato sull'intero periodo, quindi rapporta il risultato alla popolazione o all'insieme di riferimento. Tre forme richiedono di applicare prima questo test: un tasso orario o giornaliero applicato per anni, un raddoppio periodico e una crescita composta prolungata. Un risultato superiore all'insieme di riferimento stabilisce l'impossibilità soltanto a una condizione: l'**unità conteggiata deve essere non ripetibile**, cioè la stessa unità può contribuire una sola volta al conteggio in questione. Una morte per persona, un seggio per elezione, un'impresa per chiusura soddisfano questa condizione. Non la soddisfano gli eventi ripetibili — consultazioni, viaggi, transazioni — il cui numero può legittimamente superare la popolazione interessata. Quando l'insieme si rinnova durante il periodo, il limite non deve necessariamente restare chiuso: deve essere **aumentato**, cioè popolazione iniziale più un limite superiore di rinnovo. Tale limite superiore viene stabilito a partire dai dati del testo o da una verifica esterna ammissibile, mai da una stima propria dell'analista: il §5 vieta di sostituire il modello dell'autore con quello dell'analista, e una stima approssimativa del rinnovo sarebbe un modello di questo tipo. Senza un limite superiore stabilito, la moltiplicazione resta eseguibile e viene riportata, ma la conclusione del Test A sull'impossibilità **non è decidibile per mancanza di un limite stabilito**, ai sensi del §5. Il calcolo e il suo risultato vengono scritti, l'impossibilità non viene affermata, e questo silenzio non convalida la cifra. Nomina l'unità conteggiata, il limite adottato e la finestra temporale prima di concludere; senza un'unità non ripetibile, il semplice superamento non stabilisce nulla.
- **B. Denominatore.** Il denominatore è indicato? È quello corretto? Il denominatore è la popolazione, l'insieme o la base di riferimento a cui la percentuale si riferisce, non necessariamente il suo valore numerico assoluto. «9% delle vendite mondiali» indica il proprio denominatore anche se non viene fornito il volume totale delle vendite mondiali: tale volume può mancare per convertirlo in valore assoluto o per un altro test di ricostruibilità, ma la sua assenza non è un difetto di denominatore. Una percentuale senza una popolazione o base di riferimento identificabile non è un dato. Per una variazione o riduzione percentuale, il periodo o valore di confronto deve essere identificabile: «+24,5% su base annua» ha una base di confronto anche se mancano i volumi assoluti; «ridotto del 30%» senza durata, valore o periodo di riferimento non ce l'ha. Una percentuale il cui denominatore o base di confronto cambia nel corso del testo è un'equivocazione.
- **C. Misura di tendenza centrale.** Si tratta di media, mediana o moda? Il testo lo specifica? Quale delle tre cambierebbe la conclusione? Un'azienda con ventitré salari può mostrare contemporaneamente una media di 6.000, una mediana di 4.000 e una moda di 2.000, senza che nessuna delle tre cifre sia falsa.
- **D. Tasso di base.** Per ogni affermazione di rilevazione, screening, attribuzione o profilazione, viene fornita la prevalenza nella popolazione? Senza di essa, un tasso di rilevazione del 99% non dice nulla sulla probabilità che un individuo designato sia realmente interessato. L'omissione della prevalenza basta a rendere inutilizzabile un tasso di rilevazione e viene verificata sistematicamente nei testi relativi a sicurezza, salute o frode.
- **E. Misurazione, stima, modello, proiezione.** La cifra è misurata, stimata a partire da un campione, prodotta da un modello o proiettata nel futuro? Il testo distingue queste categorie? Una cifra modellizzata presentata come misurazione è un'affermazione non supportata, anche quando il modello è buono.
- **F. Precisione mostrata.** Il numero di cifre significative supera ciò che il metodo può sostenere? Una proiezione al milione più vicino su un modello decennale è falsa precisione, e la falsa precisione funziona come marcatore di serietà ai sensi di 3.K.
- **G. Riproducibilità del conteggio.** Per ogni affermazione quantitativa ottenuta selezionando o interrogando un corpus di documenti, una copertura mediatica o una banca dati documentaria, sono indicati la banca dati consultata, la query o il criterio di selezione utilizzato e l'unità di conteggio? Se manca uno qualsiasi di questi elementi, il conteggio non è riproducibile e non stabilisce nulla di verificabile, qualunque sia la plausibilità del risultato. Il test si applica al conteggio effettuato dal testo stesso e al conteggio che esso riporta quando lo presenta come stabilito e ne trae una propria conclusione. Non si applica al conteggio che il testo attribuisce esplicitamente alla propria fonte conservandone la modalità: nel secondo caso, ciò che viene esaminato non è il conteggio ma la sua appropriazione. **Limite di dominio**: G riguarda esclusivamente i conteggi documentari il cui risultato dipende dalla scelta della banca dati, della query o del criterio di selezione e dell'unità di conteggio. Non si estende a misurazioni o serie statistiche, amministrative, demografiche, scientifiche o operative ordinarie per il solo fatto che si basino su una banca dati; queste rientrano nei Test A-F e in 3.B a seconda del difetto esaminato.
...
- una cifra probatoria la cui impossibilità è stabilita dal Test A **invalida la proposizione quantitativa** che pretendeva di stabilire. La sua gravità per la tesi centrale viene determinata in 7.a mediante il test di sostituzione. Poiché il §2 qualifica ora come probatoria anche una cifra di cui la conclusione potrebbe fare a meno, tale gravità non può più essere automatica senza contraddire 7.a;
- un conteggio non riproducibile ai sensi del Test G **non stabilisce** la sottotesi empirica che sostiene, senza per questo stabilirne la falsità. Se il conteggio è effettuato dal testo stesso, la sottotesi diventa un'**affermazione quantitativa non dimostrata e non riproducibile**. Se il testo riporta un conteggio esterno che aveva presentato come stabilito, non può più utilizzarlo come risultato dimostrato e deve conservarlo come **affermazione riportata**. L'effetto sulla tesi centrale viene determinato in 7.a mediante il test di sostituzione e non è automatico;
- la parola **redibitorio** è riservata alla griglia di 7.a, dove significa che il difetto invalida la tesi centrale. La constatazione invalidante della prima regola non viene trasferita in 7.a sotto tale etichetta, e la seconda regola non stabilisce alcuna invalidazione, ma soltanto assenza di dimostrazione: entrambe entrano in 7.a attraverso il test di sostituzione, che da solo ne decide il rango.
