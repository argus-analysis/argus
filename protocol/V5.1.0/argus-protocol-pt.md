---
title: "ARGUS V5.1.0 — Protocolo completo"
lang: pt
subtitle: "Análise Rigorosa Guiada por um protocolo Universal e Sistemático"
footer: "ARGUS é um protocolo livre e gratuito, concebido por <strong>François Vadrot</strong>."
description: "Protocolo completo ARGUS V5.1.0"
page_slug: "protocolo.html"
---

**ARGUS V5.1.0 — Protocolo completo**
...
- pontos de convergência, etapa por etapa, uma linha cada um;
- pontos de divergência, etapa por etapa, com a posição de cada uma das duas análises;
- para cada divergência, a natureza do desacordo: facto, qualificação, nível de gravidade ou interpretação;
- um caso particular a assinalar sistematicamente: divergências em que uma análise conclui e a outra declara elementos insuficientes (cf. Etapa 7). Isolam o ponto exato em que uma das duas foi além do que o texto permite estabelecer;
- divergências de cobertura: constatações assinaladas por apenas uma das duas análises e não mencionadas pela outra. Trate-as separadamente das divergências de posição. Para cada uma, a análise que não a assinalou deve indicar se a rejeita depois de a examinar, se não a examinou ou se considera que vai além do que o texto permite estabelecer. Uma constatação não mencionada não é nem concedida nem contestada, e a regra de adjudicação não se lhe aplica antes dessa resposta;
- para cada divergência, o que permitiria dirimi-la e a que custo.
...
- uma marca que mostre que o conteúdo efetivamente fornecido é interrompido por um paywall, como «restam X por ler»; a simples menção «artigo reservado a assinantes» não é indício de truncatura, porque aparece também em cópias completas de artigos pagos;
- um texto que termina a meio de uma frase, de um parágrafo ou de uma enumeração;
- um programa anunciado na abertura cujas partes correspondentes estão ausentes;
- uma remissão interna para um desenvolvimento que não aparece no documento;
- uma referência a uma tabela, gráfico, nota ou anexo ausente, quando a tese depende dele;
- uma indicação de série cujos restantes elementos faltam, como «2 de 4», se o texto fornecido pressupõe os anteriores;
- um destaque introdutório que anuncia uma demonstração de que o corpo não contém qualquer elemento.
...
- a Etapa 5 não pode concluir pela existência de uma diferença entre o programa anunciado e o conteúdo efetivo, porque o conteúdo em falta pode cumprir o compromisso; a diferença observada é então declarada não conclusiva;
- o teste das ausências de 3.E não pode qualificar como omissão um elemento que a parte em falta possa conter; as ausências identificadas são declaradas sob reserva;
- o nível de 7.c, em princípio, não é atribuível no estado atual, no sentido da cláusula da Etapa 7, salvo se os elementos presentes forem suficientes, por si sós, para o estabelecer, o que deve ser justificado.
...
- **Pertinência forte**: o texto é claramente argumentativo. O protocolo pode ser aplicado integralmente.
- **Pertinência parcial**: o texto possui uma dimensão argumentativa, mas o protocolo terá de ser adaptado ou limitado a determinadas passagens. Indica ao utilizador quais as partes do texto que melhor se prestam à análise argumentativa.
Os critérios são avaliados sobre o conjunto do texto apresentado, não sobre a sua parte mais argumentativa. Quando só são preenchidos em partes identificáveis, a pertinência é parcial por composição, mesmo que essas partes preencham todos os critérios: prevalece a composição do objeto, não a força da sua parte argumentativa. A pertinência forte pressupõe, portanto, um texto argumentativo do princípio ao fim. Quando duas qualificações continuem defensáveis para o mesmo objeto, conserva a pertinência parcial por composição, que deixa aberta a possibilidade de analisar a parte argumentativa.
...
- **Pertinência fraca ou nula**: o texto não entra no uso normal de ARGUS. Propõe antes outro tipo de análise: literária, estilística, narratológica, histórica, retórica, documental ou contextual. Se o utilizador solicitar expressamente a análise apesar deste parecer, ela é realizada e começa com a menção «pertinência fraca, análise solicitada pelo utilizador». Não se trata de uma quarta saída da interrupção, mas de uma derrogação declarada, e aplica-se tanto a um texto isolado como a um texto de corpus.
...
- a qualificação de pertinência, com a sua forma e a sua razão numa linha;
- o perímetro proposto, com as secções incluídas e excluídas nomeadas;
- o regime segundo o qual as secções excluídas seriam examinadas, quando aplicável.
...
- **Perímetro já fixado pelo utilizador no pedido**: a análise adota-o sem pedir nova validação e declara no início: «perímetro fixado pelo utilizador no pedido; não é necessária validação adicional». Este perímetro é considerado validado para efeitos de 0.a bis; não é apresentado como uma decisão unilateral do analista.
- **Validação expressamente dispensada sem que o utilizador tenha fixado um perímetro**: o analista fixa autonomamente o perímetro e declara no início: «perímetro não validado pelo utilizador, fixado unilateralmente pelo analista». Não é uma formalidade: informa o leitor de que a decisão mais decisiva da análise não recebeu controlo externo.
...
- **Parcial por grau**: o texto é fracamente argumentativo no seu conjunto.
- **Parcial por composição**: o texto é plenamente argumentativo em algumas partes e didático, técnico, documental ou narrativo noutras. É o caso da maioria dos objetos reais: um relatório que contém uma tese e um anexo de dados, um dossiê de imprensa que combina editorial e reportagem, um livro em que um capítulo argumenta e vários ensinam.
...
- **Editorial / artigo de opinião**: expressão de uma opinião, intenção persuasiva. As exigências de prova factual são baixas; a análise centrar-se-á principalmente na lógica, na coerência e na retórica.
- **Reportagem / artigo de imprensa**: informação factual, investigação, testemunho. Os factos centrais (datas, lugares, números, citações) devem ser documentados ou atribuídos. As generalizações são toleradas se permanecerem razoáveis.
- **Ensaio / análise prospetiva**: exploração de uma tese, muitas vezes sem pretensão de demonstração exaustiva. As afirmações fortes sobre o futuro devem ser acompanhadas de um mecanismo causal plausível.
- **Artigo científico / técnico**: exigência máxima de provas, dados e métodos reproduzíveis.
- **Defesa de uma tese / manifesto**: defesa explícita de uma causa, parcialidade assumida. A análise centrar-se-á na coerência da argumentação e na transparência do ponto de vista.
- **Texto militante de campo**: mobilização de um público já afim, muitas vezes através de vocabulário de combate. A análise centrar-se-á no grau de fecho interpretativo.
- **Propaganda**: texto cuja organização protege a conclusão do exame autónomo do leitor através de uma combinação de seleção de factos, neutralização do contra-argumento, assimetrias, omissões ou mecanismos de autoimunização. O diagnóstico e o respetivo grau são estabelecidos em 7.c: essa etapa distingue a propaganda de Nível 3 da propaganda bloqueada de Nível 4 e trata os modos frontal ou sofisticado como descritores ortogonais ao nível. Esta classificação em 0.b descreve apenas o contrato de leitura instalado pelo texto; não é um diagnóstico. Em caso de dúvida na Etapa 0.b, conserva uma designação descritiva e reserva o diagnóstico para 7.c.
- **Outro** (especificar).
...
- **Fonte A**: nome, função, afiliação institucional, país, posição declarada sobre o tema, eventual financiamento ou ligações a partes interessadas.
- **Fonte B**: idem.
...
- **Público declarado**: o público a que o texto afirma explicitamente dirigir-se.
- **Público real provável**: o público a que de facto se dirige, atendendo às características do meio, do suporte, do registo linguístico, das referências culturais, do nível de tecnicidade, da secção, do modo de difusão e do contexto de publicação.
- **Público estratégico**: o público que o texto procura sobretudo convencer, reforçar, mobilizar, tranquilizar ou imunizar contra objeções.
- **Público repelido**: o público ou campo que o texto não procura convencer, mas desqualificar, ridicularizar ou excluir do perímetro do dizível.

Responde depois às perguntas seguintes:

- **Que códigos de credibilidade espera este público?**
  Exemplos: competência, citações, números, referências históricas, moderação aparente, ironia, indignação moral, testemunho direto, simplicidade, complexidade, tecnicidade, neutralidade de tom.

- **Que sinais fariam o texto parecer pouco credível ou demasiado propagandístico aos olhos deste público?**
  Exemplos: ausência total de contra-argumento, excesso de pathos, simplificação demasiado visível, slogans, vocabulário militante, fontes excessivamente homogéneas, acusação demasiado direta.

- **O texto introduz matizes, objeções ou concessões?**
  Se sim, abrem realmente a possibilidade de uma conclusão diferente ou servem sobretudo para proteger o texto de uma acusação de parcialidade?

- **Que objeções teria maior probabilidade de formular o público-alvo?**
  O texto enfrenta-as realmente, neutraliza-as ou evita-as?

- **O texto está calibrado para produzir um efeito de seriedade junto deste público específico?**
  Se sim, que dispositivos desempenham essa função?
...
- **Teste de definição**: o texto fornece, para esse termo, uma definição, um critério de aplicação ou uma enumeração do que abrange? Trata-se de uma constatação de presença ou ausência, sem avaliar a qualidade da definição. Uma caracterização evocativa que não forneça qualquer critério para saber se o termo se aplica a um caso determinado não satisfaz o teste: o teste é então positivo, apesar da abundância de comentários que o texto dedica ao termo. É o caso mais difícil de detetar, porque o texto parece explícito sobre uma palavra cujo limite nunca fixa.
- **Teste de fronteira**: nomeia duas delimitações possíveis do termo, uma restritiva e outra extensiva, ambas compatíveis com o uso que o texto faz dele. Reformula a conclusão sob cada uma. Se a conclusão se sustenta sob uma delimitação e falha sob a outra, em qualquer direção, a fronteira do termo sustenta a argumentação e o teste é positivo. Se a conclusão se sustenta sob ambas, o termo é vago sem ser estratégico e o teste é negativo. É o teste de substituição da Etapa 7.a aplicado à extensão de um termo e não a um elemento de prova: a correção mínima não incide sobre uma peça do processo, mas sobre o limite de uma palavra.
...
**Ponto de controlo, mapa modal (introduzido em V5.1.0).** Paralelamente ao ponto de controlo anterior, traça o mapa modal das proposições portantes. Para cada proposição cuja remoção tornaria a tese ininteligível ou não demonstrada, anota duas coisas tal como o texto as coloca **nesse ponto**: a sua **polaridade**, P ou ¬P, e o seu **grau de compromisso** com essa proposição — assertórico (colocado como facto), provável, hipotético ou suspenso (juízo deixado em suspenso). A suspensão diz respeito ao grau de compromisso com a proposição-alvo; não é uma terceira polaridade: «não se sabe se P» permanece de polaridade P, no grau suspenso. A anotação é neutra: regista-se, ainda não se julga a constância. Uma concessão é um **lugar** onde pode surgir um grau de compromisso baixo, não um grau em si. Se a mesma proposição portante, sobre o mesmo referente e com polaridade constante, surgir em vários lugares com graus de compromisso diferentes, anota cada ocorrência e o seu lugar — abertura, corpo, concessão ou reserva final. Só estas proposições, cujo grau de compromisso varia com polaridade e referente constantes, serão retomadas no controlo de constância modal da Etapa 6. A passagem limita-se às proposições portantes; não anota cada frase.
...
- As conclusões decorrem das premissas?
- Existem saltos inferenciais injustificados ou deslocações de sentido?
- As cadeias causais são demonstradas ou simplesmente postuladas?
...
- a atribuição de uma citação decisiva (autor, obra, data);
- a identidade, natureza ou posição de uma fonte nomeada (instituição, organismo, função);
- a existência, denominação exata ou natureza de uma entidade citada como prova contra alguém;
- um ponto factual isolado de que depende uma inferência e que uma única pesquisa bastaria para estabelecer.

Toda a verificação externa é acompanhada das condições seguintes:

- notificar explicitamente que a análise sai do perímetro estrito do texto;
- assegurar-se de que a verificação diz respeito ao objeto declarado no início da análise e não a outra versão ou edição do mesmo texto;
- citar a fonte externa (documento público, estudo, declaração contrária, etc.);
- reconhecer que o texto original não continha essa informação.
...
- **Estado do controlo**: *concluído*, sustentado por uma fonte segundo a condição acima; *parcial*, nomeando as partes estabelecidas e não estabelecidas; *infrutífero*, quando o controlo foi realizado sem estabelecer nenhum dos elementos procurados. Este estado não pressupõe uma pesquisa em linha: uma fonte fornecida que não estabelece nada do que se procurava produz um controlo infrutífero, e os dois eixos continuam independentes.
- **Modo de acesso à fonte**: *fonte fornecida*, confronto com um anexo ou documento já presente; *pesquisa externa*, uma fonte que foi necessário procurar.
...
- As afirmações factuais são sustentadas dentro do texto ou simplesmente enunciadas sem prova?
- Os acontecimentos são datados com precisão dentro do texto ou a cronologia é vaga, comprimida ou mesmo contraditória?
- Se for estabelecida uma tipologia ou uma hierarquia entre fenómenos, cada termo é descrito com exatidão e integralidade dentro do texto? Factos ou mediações que o próprio texto menciona noutro lugar são depois omitidos ou deformados ao serviço da tese?
- Para cada citação decisiva: são indicados a obra, a data e a página? O contexto de enunciação é aquele que o texto analisado lhe atribui? Uma citação é **recontextualizada** quando é autêntica mas deslocada para uma situação a que não se referia. Este defeito sobrevive à verificação literal, porque as palavras são exatas, e só é detetado remontando à fonte.
- **Fidelidade a uma fonte primária disponível.** O carácter primário, oficial, contemporâneo ou diretamente pertinente de uma peça fornecida não basta para ativar este teste. Só se ativa quando o próprio texto analisado se relaciona com essa fonte — cita-a, nomeia-a, resume-a, comenta-a ou critica-a — ou quando a análise pode estabelecer que a afirmação controlada reproduz o seu conteúdo. A simples concordância de tema, data, formulação ou número não basta para estabelecer essa filiação. Caso contrário, a peça continua a ser uma verificação externa comum: pode corroborar, contradizer ou não corroborar uma afirmação do texto, mas não se aplicam as qualificações de fidelidade abaixo. Quando a relação com a fonte é estabelecida, as afirmações decisivas que lhe dizem respeito devem ser confrontadas com ela. O controlo é limitado: incide sobre as afirmações de que dependem a tese do texto ou o seu contrato de credibilidade, não sobre todas as frases. Para cada uma, qualifica:
  **fiel**, o sentido, o alcance e o grau de certeza são preservados;
  **reforçada**, uma hipótese, reserva, correlação ou constatação limitada é transformada numa afirmação mais geral ou mais certa;
  **enfraquecida**, o texto atenua ou restringe aquilo que a fonte estabelece;
  **deslocada**, as palavras ou os dados são exatos mas atribuídos a um referente, comparação, período ou cena que a fonte não tratava;
  **inexata**, a fonte diz outra coisa sobre um facto verificável e decisivo;
  **interpretação própria assinalada**, o texto acrescenta uma leitura que não é a da fonte e apresenta-a explicitamente como sua;
  **não decidível**, a fonte disponível não permite concluir.
  A constatação «reforçada» merece atenção especial porque pode produzir uma afirmação mais segura do que a sua fonte, permanecendo literalmente defensável. A sua gravidade é determinada exclusivamente em 7.a pelo teste de substituição, não pela sua qualificação.
  Estas qualificações não são necessariamente exclusivas: uma afirmação pode ser simultaneamente reforçada e deslocada. Quando se acumulam, nomeia a qualificação principal e as secundárias. «Fiel» e «não decidível», pelo contrário, são incompatíveis com qualquer qualificação de defeito relativa ao mesmo elemento.
  Três das sete qualificações não são defeitos e não passam a 7.a: **fiel**, **interpretação própria assinalada** e **não decidível**. A primeira regista a conformidade, a segunda regista que o texto assume uma leitura em vez de a atribuir à fonte, e a terceira regista uma limitação da análise e não do texto. Uma fonte primária disponível não ativa, por si só, o Anexo 3 nem este teste de fidelidade. Se a fonte primária com que o texto se relaciona nas condições acima não estiver disponível, declara-o e nomeia as afirmações para as quais continua não decidível a fronteira entre aquilo que a fonte diz e aquilo que o texto acrescenta, o que corresponde à cláusula «Quando os elementos são insuficientes» da Etapa 7.
- Se o texto contiver números probatórios no sentido do §2 do Anexo 4, ativa esse anexo e apresenta as suas constatações numa subsecção específica de 3.B intitulada «Controlo aritmético e estatístico (Anexo 4)».

#### C. Utilização de termos simbolicamente carregados

- Identifica as palavras que, por si sós, convocam um registo emocional, cultural ou histórico maciço (pão, sangue, terra, o povo, pátria, liberdade, etc.).
- Pergunta se essas palavras substituem a demonstração pela evocação e se a sua força afetiva imuniza o argumento contra a contradição.
- Descrevem adequadamente a realidade ou são inadequadas apesar de retoricamente poderosas? A quem se dirigem realmente?

#### D. Utilização de termos com pretensão de universalidade

- Quando o texto utiliza «a espécie humana», «planetário», «todos», «nós», identifica a quem esses termos se referem realmente.
- O autor fala em nome de uma humanidade abstrata enquanto descreve realidades que só dizem respeito a uma parte específica dessa humanidade?
- O «nós» é uma máscara para um público particular (ocidental, europeu, etc.) apresentado como universal?

#### E. Teste das ausências

*Fase 1 — Identificação*

- Que outras explicações dos mesmos fenómenos estão ausentes do texto?
- Que factos, atores ou ligações, se fossem tidos em conta, enfraqueceriam a tese?
...
- A tese está formulada de modo a poder ser refutada ou está imunizada contra qualquer contradição?
- O texto consegue dar conta de um facto que a contradiga ou teria de o ignorar ou deformar para sobreviver?

Para além da forma da tese, examina a maneira como o texto a utiliza.

- Enumera as observações que o texto trata como confirmações.
- Pergunta se uma observação de sinal contrário é também tratada como confirmação. Se sim, qualifica o defeito como **infalsificabilidade de uso**, distinta da **imunização formal**: a tese é refutável na sua formulação, mas absorve qualquer observação na sua aplicação. Esta forma de imunização sobrevive a um exame limitado à formulação da tese, porque essa formulação é refutável: o defeito só aparece no uso.
- Pergunta explicitamente: que observação reconheceria o texto como refutação? Se não nomear nenhuma, assinala-o.

#### G. Registo estilístico e legibilidade

- Identifica as frases cuja complexidade sintática é tal que não podem ser compreendidas numa primeira leitura e são incompatíveis com a oralidade.
- Pergunta se essa complexidade é funcional (exprime uma complexidade real da realidade) ou retórica (produz um efeito de autoridade intelectual, filtra o público leitor ou mascara uma impropriedade lógica).
- Confronta o registo linguístico com o destinatário declarado do texto. Se o texto afirma dirigir-se a «todos», fala numa linguagem realmente acessível a todos? A distância entre o «nós» proclamado e o registo real constitui uma contradição performativa a assinalar.
...
- O mesmo defeito pode ser encontrado em quem fala, no grupo ou instância que privilegia ou no próprio dispositivo do texto (enquadramento, omissões, pressupostos)?
- Se sim, o texto reconhece-o? Retira as consequências?
- Se não, a diferença de tratamento é explícita e validamente justificada?
...
- **Existe uma assimetria sistemática no tratamento dos diferentes atores?** Se sim, o texto justifica-a explicitamente? Se não fornecer uma justificação válida, assinala essa assimetria como fraqueza argumentativa.
...
- **O texto utiliza os marcadores de seriedade esperados pelo seu público-alvo?**
  Numerosas fontes, especialistas, referências históricas, tom analítico, prudência lexical, números, concessões, complexidade sintática, distinção aparente entre factos e interpretações.

- **Esses marcadores reforçam realmente a qualidade demonstrativa do texto ou servem sobretudo para tornar aceitável uma conclusão já fixada?**

- **O contra-argumento é formal ou efetivo?**
  Um contra-argumento é efetivo se obrigar o texto a modificar, limitar ou reconfigurar a sua tese.
  Um contra-argumento é formal (ou decorativo) se for mencionado mas imediatamente absorvido, marginalizado ou neutralizado.

- **As concessões abrem ou imunizam?**
  Uma concessão de abertura torna possível uma conclusão diferente ou mais incerta.
  Uma concessão imunizante produz uma aparência de equilíbrio ao mesmo tempo que protege a conclusão central da crítica.

- **As omissões já identificadas em 3.E são precisamente aquelas que o público-alvo identificado em 0.f precisaria de conhecer para avaliar a tese de forma autónoma?**

- **O texto produz fecho interpretativo sob a forma de complexidade?**
  Por outras palavras: dá ao leitor a sensação de ter atravessado a complexidade, enquanto os elementos introduzidos nunca ameaçam realmente a conclusão?
  Num texto compósito, este teste incide sobre o perímetro argumentativo declarado em 0.a bis, não sobre as secções técnicas ou didáticas. Uma dificuldade técnica real que não serve nenhuma conclusão não é fecho interpretativo.

- **Onde se concentra o rigor?**
  Localiza separadamente, no texto, as afirmações acompanhadas de uma referência precisa (obra, data, página, fonte nomeada, método declarado) e as afirmações sem referência. Compara depois as duas localizações. As referências precisas incidem sobre afirmações que o público-alvo pode verificar autonomamente a baixo custo ou sobre aquelas que sustentam a tese?
  Um rigor concentrado no verificável e ausente do decisivo funciona como dispositivo de credibilidade e não como justificação. Produz um efeito de erudição mais amplo do que o apoio documental real, e é tanto mais eficaz quanto mais impecáveis forem as poucas referências fornecidas.

Termos recomendados na análise:

- **Contra-argumento efetivo**: presença de uma objeção realmente levada a sério.
- **Contra-argumento decorativo**: presença de uma objeção que não modifica a trajetória argumentativa.
- **Concessão imunizante**: matiz que serve sobretudo para evitar uma acusação de parcialidade.
- **Matiz bloqueado**: matiz integrado de forma a reforçar a conclusão em vez de a abrir.
- **Propaganda adaptada**: propaganda que adota os códigos intelectuais, morais ou estéticos do público a que se dirige.
- **Rigor deslocado**: precisão documental concentrada nas afirmações mais fáceis de verificar e ausente daquelas que sustentam a tese.
...
- marcas de distinção ou posicionamento dentro de um campo (referências a escolas de pensamento, a «más maneiras» de pensar, a adversários não nomeados);
- dirigir-se a um público específico (marcas de cumplicidade, pressupostos partilhados, um «nós» cujo perímetro real deve ser identificado);
- silêncios, omissões, temas expectáveis que são evitados;
- insistências, repetições, formulações que parecem procurar um efeito preciso sobre o leitor (culpabilização, mobilização, legitimação do autor);
- a distância entre o registo linguístico e o destinatário declarado;
- tensões, contradições ou ambiguidades que sugerem que o texto não pode assumir abertamente tudo aquilo que faz.
...
- o destinatário real (a quem fala realmente o texto? Dirige-se aos já convencidos para os reforçar mais do que aos indecisos para os persuadir? Fala aos seus pares para marcar uma posição num campo intelectual mais do que ao grande público para o mobilizar?);
- a função de legitimação do autor (o texto serve para conferir ao autor uma posição de especialista, teórico, figura moral ou porta-voz?);
- o efeito procurado no leitor (informar, comover, culpabilizar, mobilizar, paralisar, reforçar, silenciar) e a sua coerência com a finalidade declarada;
- uma operação de distinção ou posicionamento dentro de um campo intelectual ou político;
- uma função estratégica implícita (por exemplo: desqualificar um concorrente sem o nomear, distanciar-se de uma posição mais radical ou mais moderada, proteger uma posição institucional, produzir um efeito de legitimação sem o declarar explicitamente).

**Nível 3 — Grau de confiança**

Para cada hipótese, avalia o grau de confiança que o texto permite, segundo uma escala simples:

- **Alto**: a hipótese é sustentada por indícios convergentes e nenhum elemento do texto a contradiz.
- **Médio**: existem indícios, mas são possíveis outras interpretações.
- **Baixo**: a hipótese assenta em indícios ténues ou ambíguos; é plausível, mas está longe de ser certa.
...
- O texto faz aquilo que disse que faria?
- Abstém-se daquilo que disse que não faria?
- Se for constatada uma diferença, a declaração inicial deve ser requalificada: funciona como dispositivo de autolegitimação. A diferença estabelece essa função; por si só, não permite concluir nada sobre a sinceridade do autor, e a análise não deve fazê-lo.
...
- O enquadramento inicial predeterminou ou protegeu a argumentação neutralizando antecipadamente determinadas objeções?
- A tese sobrevive se o postulado de abertura for rejeitado?
- Os argumentos são independentes do enquadramento ou dependem inteiramente dele?
- A intenção estratégica inferida lança nova luz sobre a função do dispositivo de abertura?

**Controlo de constância modal (introduzido em V5.1.0).** A partir do mapa modal traçado na Etapa 2, examina cada proposição portante cujo **grau de compromisso varia com polaridade e referente constantes**. Este controlo compara o grau de compromisso com uma mesma proposição, não a sua polaridade: capta uma incoerência que o exame da validade lógica (Etapa 3) deixa passar porque não é uma contradição, mas um enfraquecimento do grau de compromisso.

Aplica o **teste de direção**:

1. É o **grau forte** — assertórico, ou o mais elevado dos dois graus observados — aquele que realiza o trabalho argumentativo, aquele sob o qual a proposição serve de premissa, reforça a tese ou incrimina um adversário?
2. Surge o **grau fraco** — suspenso ou hipotético, ou um recuo para «provável» a partir do assertórico — onde um compromisso forte teria um custo: secção sobre as divergências, concessão, reserva final, resposta a uma objeção?

Dois «sim»: **retratação modal direcional**. O texto coloca a proposição como adquirida onde rende e retira-a onde estabelecê-la custaria. A gravidade determina-se em 7.a através do teste de substituição, salvo as três exceções seguintes.

**Exceções (extensão da Regra 14).** Cada uma, se se aplicar, não é um defeito e o controlo permanece mudo.

- **Modéstia constante.** A proposição mantém o mesmo grau de compromisso prudente do princípio ao fim; a reserva final apenas restitui essa prudência constante. Verificação: a abertura já é prudente? Nesse caso não há retratação. É a salvaguarda que a Regra 14 já protege.
- **Revisão assumida.** O autor coloca a proposição no grau forte, depois revê-a explicitamente **e deixa efetivamente de se apoiar na sua força para concluir**, ajustando a sua conclusão. As duas condições são cumulativas. O reconhecimento explícito não basta: a revisão não se verifica se a conclusão reafirmar a mesma força sob uma formulação mais estreita, reformulada ou reenquadrada, em particular se mantiver a recomendação ou a conclusão que a afirmação forte sustentava. Reconhecer que uma afirmação era «demasiado absoluta» e depois continuar a retirar dela o mesmo benefício não é uma revisão assumida mas uma retratação disfarçada: o controlo é acionado. Verificação de aplicação: antes de conceder a exceção, verifica que a conclusão não volta a transportar a força do polo forte, literal ou reformulada; se a recomendação que dependia do polo forte sobrevive intacta, a exceção não se aplica.
- **Partição do perímetro.** Os dois polos não incidem sobre o mesmo referente ou o mesmo perímetro. Nomeia explicitamente o referente de cada polo. Se o texto pretender distinguir dois referentes — uma superfície e um substrato, uma finalidade e uma eficácia —, essa distinção deve ser demonstrada separadamente; afirmada sem demonstração, não permite aplicar a exceção. Caso de uma finalidade e de uma eficácia: a partição limpa — finalidade mantida do princípio ao fim e apenas a eficácia deixada em suspenso — é muda por construção, tendo a finalidade um grau de compromisso constante; a exceção só é invocada, portanto, se a própria finalidade variar, e verifica-se então que o corpo não tornou já incerta a finalidade, o que excluiria a leitura «finalidade adquirida, apenas a eficácia discutida».

**Fronteira com o exame da validade (Etapa 3).** Uma mudança de **polaridade** com grau forte constante — assertórico-P, depois assertórico-¬P sobre o mesmo referente — não é uma retratação modal mas uma **contradição**, alcançável pelo exame da validade na Etapa 3; o presente controlo não a trata e só pode assinalar a sua estrutura direcional. O controlo incide exclusivamente sobre o **enfraquecimento do grau de compromisso** de uma proposição cuja polaridade e referente não mudam. Tal enfraquecimento escapa ao exame da validade por falta de contradição estrita: é este o contributo próprio do controlo. O caso mais útil é a retratação disfarçada de mudança de enquadramento: a proposição forte é colocada num plano (teleologia, tendência ou intenção) e enfraquecida noutro (conteúdo ou facto presente); o controlo impõe a alternativa demonstração-ou-retirada.

**Endurecimento modal, do fraco para o forte.** O caso simétrico inverso — uma proposição portante prudente na abertura e asseverada na conclusão — não entra neste controlo. As suas formas documentada e cifrada já são captadas, uma pelo teste de fidelidade de 3.B, constatação «reforçada», a outra pelo Teste E do Anexo 4. A forma restante — o autor endurece a sua própria afirmação não cifrada sem que o corpo tenha fornecido o novo apoio que justificaria a escalada — é assinalada sob o exame da validade (Etapa 3) como uma escalada não justificada, um excesso do conclusivo sobre as premissas, reutilizando o mapa modal da Etapa 2.

**Restituição.** Uma linha de constância modal por cada proposição portante cujo grau de compromisso varia, com o veredicto — retratação modal direcional ou exceção aplicada e qual. O defeito comprovado transita para 7.a. Uma concessão que seja o polo retirado de uma retratação comprovada não pode ser creditada ao texto em 7.b. Este controlo não distingue uma retratação de um único autor de uma retratação nascida da justaposição de estratos heterogéneos do texto: essa distinção diz respeito a uma história de produção invisível no objeto, e inferi-la equivaleria a especular sobre a autoria, o que a Etapa 4 proíbe. O objeto apresenta a contradição, qualquer que seja a sua fabricação. Quando a descontinuidade é manifesta no próprio objeto — uma legenda ou uma imagem que afirma o que o corpo concede, uma caixa estilisticamente descontínua —, o analista pode acrescentar uma nota puramente descritiva que localize a contradição, sem imputar qualquer autoria ou intenção.

**Limite declarado.** A fronteira entre revisão assumida e retratação disfarçada permanece, em última instância, uma questão de juízo: um texto pode reconhecer uma afirmação demasiado forte com habilidade suficiente para que a manutenção da conclusão sobre a sua força passe por um recuo legítimo. O controlo pode então ser acionado menos do que deveria. Esta falha é conservadora: erra do lado que protege a revisão honesta. Um veredicto emitido perto desta fronteira é dado com confiança assinalada e exige uma releitura, em conformidade com a nota de fiabilidade.
...
- **Em 7.a**: se a gravidade de um defeito não puder ser estabelecida pelo teste de substituição por falta de informação, o defeito é enumerado com a nota «gravidade não determinável no estado atual», em vez de ser colocado por defeito na categoria «menor».
- **Em 7.b**: se o texto não estabelecer nada de forma sólida, escreve-o. A secção é obrigatória; preenchê-la não é. Atribuir ao texto qualidades que não possui para cumprir formalmente a secção seria uma complacência simétrica à severidade artificial.
- **Em 7.c**: se os elementos reunidos em 0.d, 0.e, 0.f, 3.I, 3.J, 3.E e 3.K — as sete etapas em que 7.c se constrói — não permitirem atribuir um nível, escreve «nível não atribuível no estado atual», indicando o elemento em falta. Não atribuas por defeito o Nível 1.
...
- **Redibitório**: o defeito invalida a tese central (por exemplo: contradição lógica maior, omissão estruturante sem a qual a tese não se sustenta, falsificação manifesta).
- **Maior**: o defeito enfraquece seriamente a tese, mas não a invalida completamente.
- **Menor**: o defeito não afeta a conclusão global do texto.
...
- Se a conclusão central deixar de poder ser mantida com a mesma pretensão: **redibitório**.
- Se a conclusão puder ser mantida, mas com uma pretensão claramente enfraquecida (menos certeza, menos generalidade): **maior**.
- Se a conclusão e a sua pretensão permanecerem inalteradas: **menor**.

**Aplicação à retratação modal (V5.1.0).** Quando o defeito é uma retratação modal comprovada (Etapa 6), a versão mínima força o grau de compromisso da proposição a um único valor constante, e a forçagem incide sobre a pretensão **tal como anunciada pelo polo forte**, não sobre uma versão benevolamente enfraquecida da tese. Se o polo forte anunciava uma afirmação factual ou uma demonstração e a forçagem a reduz a uma hipótese, a pretensão anunciada muda de natureza: redibitório para essa pretensão. Se noutro lugar sobreviver uma tese mais fraca, a sua gravidade é indicada separadamente, muitas vezes maior. Os dois níveis são nomeados onde divergem.
...
**Interação com o controlo de constância modal (V5.1.0).** Uma concessão que constitui o polo retirado de uma retratação modal comprovada (Etapa 6, exceções afastadas) não pode ser creditada ao texto em 7.b — nem como contra-argumento efetivo, nem como concessão de abertura, nem como ponto de rigor: isso equivaleria a creditar a própria metade do defeito assinalado em 7.a. O controlo de coerência da Etapa 8 verifica que nenhuma qualidade creditada em 7.b coincide com um polo retirado.

Distingue depois explicitamente:

- **As qualidades reais do texto**, tal como enumeradas em 7.b, se existirem independentemente dos seus defeitos argumentativos. Atenção: não chames «força retórica» a uma eficácia obtida por falácias. Não chames «coerência interna» ao fecho circular de um sistema sobre si próprio. Não chames «capacidade de nomear fenómenos reais» a uma evocação seletiva que omite factos contrários.

- **Os defeitos argumentativos precisos**, com o respetivo nível de gravidade (7.a).

- **As intenções estratégicas inferidas**, apresentando-as como hipóteses baseadas na análise, não como certezas. Recorda o grau de confiança associado a cada hipótese.

- **Articulação entre intenção inferida e julgamento global:**
  Se for constatada uma diferença significativa entre a finalidade declarada e a finalidade inferida, o que revela isso sobre a natureza do texto? Um texto pode ser bem-sucedido como ato político (posicionamento, mobilização) e falhar como demonstração.
  A intenção inferida não refuta a tese, mas pode explicar as suas fraquezas argumentativas: um autor que prossegue um objetivo estratégico pode sacrificar rigor à eficácia retórica, não por acidente, mas por coerência com a sua finalidade real.
  Não confundas a análise da função estratégica do texto com a refutação da sua tese.

- **Assimetrias injustificadas**: quando aplicável, enumera os defeitos que o texto atribui ao seu objeto mas não reconhece em si mesmo ou no seu próprio campo, sem justificação válida.

- **Julgamento global**: a tese está estabelecida? O texto cumpre a sua pretensão demonstrativa? Se o texto falhar como demonstração mas funcionar como manifesto ou ato de posicionamento, declara-o explicitamente, sem confundir os dois registos de avaliação. Indica se foram identificados defeitos redibitórios.
...
- **Nível 0 — Informação aberta / não propagandística**: o texto tem um enquadramento, como qualquer texto, mas nenhum viés estruturante protege a sua conclusão. As fontes são suficientemente diversas para o objeto, os atores comparáveis são tratados simetricamente e os elementos contrários podem efetivamente modificar, limitar ou inverter a conclusão.
- **Nível 1 — Informação orientada**: um enquadramento, uma seleção ou hierarquia dos factos estruturante, ou fontes marcadamente unilaterais, orientam a leitura. O leitor conserva, contudo, os elementos necessários ao exame autónomo; o contra-argumento não é sistematicamente neutralizado e uma observação contrária ainda pode afetar a conclusão. O texto orienta mais do que bloqueia.
- **Nível 2 — Argumentação de parte / jornalismo empenhado**: o texto defende ativamente uma posição ou conclusão e organiza ao seu serviço a seleção, hierarquia ou interpretação dos factos. O viés pode ser explícito ou inferível, mas as objeções continuam capazes de modificar ou limitar a conclusão. O texto procura persuadir; ainda não protege sistematicamente a conclusão contra a contradição.
- **Nível 3 — Propaganda**: vários mecanismos convergentes protegem a conclusão contra o exame autónomo — forte assimetria, circularidade informativa, contra-argumento neutralizado, concessões imunizantes, omissões estratégicas, rigor deslocado ou outros mecanismos de fecho. O leitor ainda pode reconstruir uma contradição, mas o dispositivo torna-o sensivelmente mais difícil ou absorve-a quando surge. O termo «propaganda» começa neste nível.
- **Nível 4 — Propaganda bloqueada**: a proteção torna-se autoimunizante ou materialmente fechada. Omissões estruturantes, fabricação ou manipulação decisiva, exclusão do contra-argumento ou um mecanismo que transforma toda a observação contrária em confirmação impedem a contradição interna de modificar a conclusão sem um contributo externo substancial. O exame autónomo deixa de ser apenas orientado ou dificultado: fica impedido no ponto central.
...
- De 0 para 1: aparece uma orientação estruturante?
- De 1 para 2: o texto defende ativamente uma conclusão, para além do enquadramento que organiza a informação?
- De 2 para 3: a conclusão está protegida contra a contradição por mecanismos de neutralização ou fecho?
- De 3 para 4: essa proteção torna-se autoimunizante ou materialmente tal que a contradição já não pode produzir o seu efeito sem um contributo externo substancial?
...
- **Modo retórico**: *frontal* quando o empenhamento, a acusação ou a mobilização são imediatamente visíveis; *sofisticado* quando a persuasão opera principalmente através dos códigos de credibilidade esperados pelo público — tecnicidade, competência, moderação, abundância documental, concessões ou neutralidade de tom.
- **Origem ou função**: *jornalística*, *militante*, *empresarial*, *institucional*, *estatal*, *paraestatal* ou outra qualificação estabelecida com precisão. Uma origem estatal ou institucional não implica nenhum nível automático; uma comunicação institucional aberta à contradição pode estar menos fechada do que um texto privado autoimunizante.
...
- Diversidade de fontes (0.d)
- Transparência sobre o contexto de produção (0.e)
- Identificação do público-alvo e do contrato de credibilidade (0.f)
- Circularidade informativa (3.I)
- Simetria no tratamento dos atores (3.J)
- Presença e natureza das omissões estratégicas (3.E)
- Função real dos matizes, concessões e moderações (3.K)
- Contra-argumento efetivo ou decorativo (3.K)
- Adaptação dos métodos persuasivos às expectativas do público-alvo (3.K)
- Grau de fecho interpretativo produzido pelo texto

Apresentação na análise:

Na secção 7.c, o analista deve:

- Apresentar a escala reproduzindo os cinco níveis e as respetivas etiquetas.
- Atribuir um nível ao perímetro ARGUS do texto analisado, indicando o número e a etiqueta. Para um texto não compósito, esse perímetro é o texto inteiro.
- Acrescentar, se estiverem estabelecidos e forem úteis, o modo retórico e a origem ou função como descritores separados do nível.
- Justificar a atribuição enumerando os critérios observados, com referências às etapas correspondentes (0.d, 0.e, 0.f, 3.I, 3.J, 3.E, 3.K).
- Justificar explicitamente o limiar superior: dizer por que razão o texto ultrapassa ou não ultrapassa o nível seguinte. A dúvida entre dois níveis resolve-se pela pergunta correspondente ao limiar, nunca fazendo uma média.
...
- o nível global não é a média dos níveis de subperímetro; deve ser justificado como um julgamento próprio, referido ao **perímetro ARGUS considerado como um todo**, e não às secções excluídas em 0.a bis;
- cada nível de subperímetro deve estar ligado a secções nomeadas, nunca a uma passagem isolada selecionada a posteriori. Sem esta condição, a subatribuição permitiria extrair um parágrafo severamente qualificado de um texto de resto sólido, produzindo uma imagem não representativa do texto.
...
- **Nível 0 — Informação aberta / não propagandística**: enquadramento sem proteção estrutural da conclusão.
- **Nível 1 — Informação orientada**: orientação estruturante, exame autónomo ainda efetivo.
- **Nível 2 — Argumentação de parte / jornalismo empenhado**: conclusão ativamente defendida, objeções ainda capazes de a modificar.
- **Nível 3 — Propaganda**: conclusão protegida por mecanismos convergentes de neutralização ou fecho.
- **Nível 4 — Propaganda bloqueada**: fecho autoimunizante ou material que impede a contradição de produzir o seu efeito central.

**Nível atribuído ao perímetro ARGUS analisado**: Nível [0–4] — [etiqueta].
**Modo retórico, se estabelecido**: [frontal / sofisticado].
**Origem ou função, se estabelecida**: [jornalística / militante / empresarial / institucional / estatal / paraestatal / outra].

**Justificação**:

- Diversidade de fontes (0.d): [constatação].
- Transparência sobre o contexto de produção (0.e): [constatação].
- Público-alvo e contrato de credibilidade (0.f): [constatação].
- Circularidade informativa (3.I): [constatação].
- Simetria de tratamento (3.J): [constatação].
- Omissões estratégicas (3.E): [constatação].
- Contra-argumento, concessões e adaptação ao público (3.K): [constatação].
- **Limiar superior**: [razão pela qual o nível seguinte é ou não ultrapassado].
...
- **Cabeçalho e anexos**: os anexos declarados ativos no cabeçalho são aqueles que os pontos de controlo decidiram, e cada um produziu efetivamente a respetiva secção?
- **Verificações externas**: os três números declarados coincidem com os controlos apresentados no corpo da análise, cada um no seu próprio total, e esses controlos incidem sobre o objeto declarado e não sobre outra versão do texto?
- **Não contradição entre etapas**: uma afirmação da etapa 7 contradiz uma constatação das etapas 3 a 6? Em particular, pode um elemento identificado como ponto de rigor do texto coexistir com uma qualificação que pressupõe a sua ausência?
- **Constância modal**: são coerentes entre si o mapa modal da Etapa 2 e o veredicto de constância modal da Etapa 6? Quando foi assinalada uma retratação modal, foi indevidamente creditada ao texto em 7.b uma concessão que é o seu polo retirado?
- **Cálculos**: todo o cálculo transferido para 7.b como exato e verificado é acompanhado do valor recalculado pelo analista? Se não, é declarado não verificado e não exato.
- **Constatações factuais**: o mesmo facto recebe duas descrições incompatíveis em dois pontos da análise?
- **Fonte primária**: quando a relação entre o texto e uma fonte primária satisfazia as condições de ativação do teste de fidelidade de 3.B, as afirmações decisivas foram confrontadas com essa fonte e o resultado foi qualificado segundo o teste? Uma peça primária simplesmente fornecida ou pertinente foi indevidamente tratada como fonte do texto sem filiação estabelecida?
- **Qualificação das ausências**: foi recusada a uma ausência a qualificação de omissão estratégica com o fundamento de que não destruiria a tese, critério que pertence apenas à segunda porta de 3.E? Uma ausência foi colocada na saída «não determinável no estado atual» apesar de um controlo razoavelmente acessível ter permitido decidir e não ter sido tentado, ou sem declarar a indisponibilidade de qualquer controlo? Esta saída proíbe ambas as coisas.
- **Registo da análise** (Regra 19): a análise contém elogios ou censuras comparativas que não posso estabelecer, ou um termo carregado que a constatação não exige?
...
- (a) um pressuposto não demonstrado admitido pelo próprio analista;
- (b) um risco de viés identificado (incluindo viés de série ou de ancoragem se a análise fizer parte de uma sequência de textos);
- (c) uma limitação de informação não resolvida;
- (d) **deferência**: o estatuto de uma fonte (instituição reconhecida, autor canónico, revista com revisão por pares, agência de notícias estabelecida) reduziu a exigência probatória do analista sobre uma afirmação concreta? Se sim, qual?
- (e) **conformismo de série**: se esta análise sucede a outras análises, o analista alinhou um julgamento com os anteriores em vez de o alinhar com o texto? Sobre que ponto exato?
...
- **Pressupostos da análise**: que pressupostos não demonstrados admitiu o próprio analista (por exemplo, superioridade de um tipo de prova sobre outro, legitimidade da exigência de fontes externas, etc.)?
- **Simetria epistémica**: o analista aplicou ao seu próprio discurso as mesmas exigências de rigor, transparência e não contradição que exigiu ao texto estudado?
- **Vieses potenciais**: o analista tem razões para suspeitar que as suas próprias opiniões (políticas, profissionais, culturais) orientaram a análise?
- **Limites da análise**: que informação adicional seria necessária para resolver determinados pontos que permaneceram indeterminados?
...
14. Nunca atenuares uma conclusão crítica com moderadores vagos ou tímidos quando o texto não justificar essa atenuação. Expressões como «insuficientemente», «não suficientemente», «relativamente», «parcialmente», «tende a», «parece», «quase», «em certa medida» só podem ser utilizadas se corresponderem exatamente ao estado do texto. Se um elemento estiver ausente, diz que está ausente. Se for observada uma contradição, diz que existe uma contradição. A prudência crítica não consiste em enfraquecer o diagnóstico, mas em distinguir corretamente aquilo que a análise permite afirmar daquilo que não permite afirmar.
   ...
   Esta regra proíbe o analista de utilizar moderadores para evitar o seu próprio julgamento. Não deve levar a penalizar um autor cujo texto exprima por si só matizes, prudência ou explore hipóteses alternativas. Se o texto for matizado, a análise deve reproduzir esse matiz em termos precisos, sem o transformar numa acusação de vagueza. O controlo de constância modal da Etapa 6 estende esta mesma salvaguarda à modalidade de uma proposição portante: não é acionado nem perante uma prudência constante, nem perante uma revisão explicitamente assumida.

15. Regras de formatação da saída da análise:
  - Nunca uses tabelas (nem Markdown nem HTML). Usa listas com pontos, listas numeradas, pares chave-valor ou parágrafos estruturados com subtítulos.
  - Para as listas com pontos, usa exclusivamente o caráter `-` como marcador, com uma linha em branco antes do primeiro ponto, um espaço depois do traço e sem linhas em branco entre os elementos. As listas numeradas também são permitidas segundo o mesmo princípio de espaçamento (linha em branco antes, sem linhas em branco entre elementos).
  - Para os subelementos, não uses pontos secundários. Coloca a informação suplementar nas linhas seguintes, sem marcador, com dois espaços de indentação.
  - Para pares chave-valor, usa o formato `**Termo**: definição.`

16. **Nunca confundas contra-argumento formal com contra-argumento efetivo.** A presença de um matiz, de uma concessão ou de uma fonte contrária não demonstra equilíbrio do texto. O analista deve verificar se esse matiz pode realmente modificar a conclusão ou se serve apenas para a tornar mais aceitável para o público-alvo.

17. **Avalia sempre a propaganda em relação ao seu público-alvo.** Uma propaganda dirigida a um público culto pode ser documentada, matizada, prudente e estilisticamente sofisticada. Estas qualidades formais não a excluem do campo da propaganda se servirem para fechar a interpretação em vez de a abrir.

18. **Nunca produzas uma análise artificialmente completa.** Se o texto não fornecer elementos suficientes para tratar uma secção de forma fiável, o analista deve declará-lo explicitamente («Dados insuficientes», «Não existem elementos probatórios», «Questão não decidível apenas a partir do texto»). É preferível reconhecer um limite a preencher uma secção com especulação ou generalidades. A aplicação detalhada desta regra, com os seus três limites, figura no início da etapa 7 sob a cláusula «Quando os elementos são insuficientes».

19. **Aplica os Testes 3.C e 3.G à tua própria prosa.** O protocolo examina o registo do texto analisado; examina também o da análise. Uma análise crítica que não vigia o seu próprio registo é contradita pela própria prosa antes de ser contradita pelas suas constatações. Daqui resultam duas proibições.
  **Elogio ou censura comparativa não estabelecida.** Creditar ou desacreditar um texto comparando-o com uma classe de textos («melhor do que o género exige», «raro neste tipo de produção», «como acontece frequentemente nestes meios») pressupõe uma afirmação empírica sobre essa classe. Se a análise não puder estabelecê-la, a comparação é eliminada e a constatação formulada sem contraste. Esta regra é a contrapartida da Regra 14: aquela proíbe moderadores vagos que suavizam a crítica; esta proíbe comparações vagas que inflam um elogio ou uma censura.
  **Termos carregados na análise.** O vocabulário identificado por 3.C no texto analisado não tem lugar no comentário que o identifica. Teste operacional: elimina o termo pejorativo e verifica se a constatação se mantém. Se se mantiver, o termo era supérfluo; se não, a constatação não estava estabelecida.
...
- A norma de prova declarada antes do exame e indexada ao género textual. Uma lista de falácias aplicada sem ter em conta o género do texto não fornece essa indexação: trata da mesma forma um editorial e um artigo científico, embora os seus contratos de leitura sejam diferentes.
- A separação, na etapa 7.c, entre o **grau de fecho interpretativo** (Níveis 0 a 4) e os descritores de **modo retórico** (frontal ou sofisticado) e de **origem ou função**. Uma propaganda dirigida a um público culto pode ser documentada, matizada e prudente; estes traços descrevem a sua forma sem determinarem, por si sós, o seu grau.
- A distinção entre contra-argumento formal e efetivo, e entre concessões de abertura e imunizantes.
- A secção obrigatória 7.b, que impede que o protocolo funcione como uma máquina destinada a concluir contra.
- A etapa 8, para a qual não encontrei equivalente nos instrumentos consultados.
...
As etapas avançadas 0.d, 0.e, 0.f, 3.I, 3.J, 3.K e 7.c, juntamente com os Anexos 2 e 3, não estão incluídas na versão Light. Do Anexo 4, apenas os Testes A e G são incluídos; os Testes B a F ficam excluídos. O controlo de constância modal — o mapa modal da Etapa 2 e o controlo da Etapa 6 introduzidos em V5.1.0 — não está incluído na versão Light.
...
- unir um público com interesses diversos,
- evitar uma falsificabilidade demasiado rápida,
- produzir um efeito de evidência ou urgência sem ter de o justificar,
- desqualificar um adversário por associação a um termo vago mas carregado.
...
- **O termo nunca é definido** no texto, ou as suas definições sucessivas são incompatíveis. Exemplo genérico: «justiça», «o povo», «verdadeira democracia».
- **O termo serve para reunir reivindicações heterogéneas** sem as precisar. Exemplo genérico: «queremos mudança».
- **O autor opõe dois significantes vazios** para criar uma fronteira moral («nós» contra «eles»). Exemplo genérico: «o interesse geral» contra «os privilégios».
- **O termo tem uma forte carga emocional** mas um fraco conteúdo informativo. Exemplo genérico: «escândalo», «catástrofe», «urgência absoluta».
- **A argumentação assenta na evocação desse termo** sem necessidade de o demonstrar. Exemplo genérico: «o momento histórico», «a janela», «o impulso necessário».

**3. Grelha de análise de um significante vazio**

Para cada termo suspeito, aplica sistematicamente:

- **Localização**: Onde aparece? Em que momentos estratégicos (abertura, conclusão, transição)?
- **Estatuto**:
  - **Vazio**: ausência de definição, uso intercambiável.
  - **Cheio**: definido, estável, não substituível sem perda de sentido.
  - **Misto**: vazio em alguns pontos, cheio noutros (tensão potencial).
- **Função no argumento**:
  - Unir (agrupar coletivos diferentes)
  - Evitar a refutação (tornar a tese não falsificável)
  - Sugerir legitimidade sem prova
  - Criar uma sensação de urgência
  - Distinguir o autor de um adversário (rejeitando um significante vazio concorrente)
- **Diferença em relação a um possível uso cheio**: Se o mesmo termo for usado como significante cheio noutro lugar (pelo autor ou no campo discursivo), a diferença é justificada? Assinala uma contradição?
- **Impacto sobre a solidez argumentativa**:
  - **Baixo**: o vazio é anedótico, a tese mantém-se sem ele.
  - **Médio**: a tese depende parcialmente dessa vagueza (por exemplo, previsão vaga).
  - **Alto**: sem o significante vazio, o argumento desmorona (por exemplo, «urgência» sem prova).

**4. Ligação às etapas do protocolo ARGUS**

- **Etapa 1 – Dispositivo de abertura**: A abertura pode instalar um ou mais significantes vazios («crise», «urgência», «nós») que enquadram todo o texto.
- **Etapa 3.C – Termos com carga simbólica**: Distingue os termos carregados mas cheios (por exemplo, «greve») dos termos carregados e vazios (por exemplo, «justiça»). Isto afina a crítica.
- **Etapa 3.E – Teste das ausências**: Um significante vazio pode mascarar omissões: a vagueza permite não escolher entre soluções contraditórias.
- **Etapa 3.F – Falsificabilidade**: Um significante vazio torna a tese difícil de falsificar, porque o conteúdo ausente pode ser redefinido a posteriori.
- **Etapa 3.H – Simetria epistémica**: Um texto que utiliza significantes vazios para desqualificar um adversário utiliza, por sua vez, significantes vazios para se legitimar?
- **Etapa 4 – Intenção estratégica**: O uso de significantes vazios pode constituir um indício de uma intenção de mobilizar mais do que demonstrar; o seu peso é determinado juntamente com os restantes indícios da etapa 4.
- **Etapa 7 – Conclusão diferenciada**: Indica se a tese central assenta em um ou mais significantes vazios injustificados.
...
- **Termo «crise»**: não definido (económica? política? moral?). Intercambiável por «perigo», «emergência». Estatuto: **vazio**. Função: criar uma sensação de urgência sem prova.
- **Termo «povo»**: não definido (nacional? local? os oprimidos? todos os cidadãos?). Estatuto: **vazio**. Função: unir reivindicações heterogéneas em torno de um «nós» vago.
- **Termo «janela»**: sem conteúdo temporal nem processual. Estatuto: **vazio**. Impacto: **alto** — se estes três significantes vazios forem eliminados, resta apenas uma exortação sem justificação.
...
- O analista deve registar, no fim de cada análise individual, um breve resumo dos níveis atribuídos em 7.a (gravidade) e 7.c (grau de propaganda), para construir uma grelha comparativa em C.3.
- Se a ordem de análise não for neutra (por exemplo, se o corpus for fornecido numa ordem cronológica ou temática escolhida por um terceiro), o analista deve assinalá-la como possível fator de ancoragem (ver C.3).
...
- um número, taxa, proporção ou projeção probatória no sentido definido abaixo;
- uma comparação quantitativa entre dois conjuntos;
- uma contagem relativa a um corpus, a uma cobertura mediática ou a uma base documental;
- uma afirmação de deteção, rastreio, atribuição ou perfilagem;
- uma média apresentada como característica de uma população.
...
- **A. Ordem de grandeza.** Extrapola a taxa indicada para todo o período e relaciona depois o resultado com a população ou conjunto de referência. Três formas exigem a aplicação prioritária deste teste: uma taxa horária ou diária aplicada durante anos, uma duplicação periódica e um crescimento composto prolongado. Um resultado que ultrapassa o conjunto de referência só estabelece impossibilidade sob uma condição: a **unidade contabilizada deve ser não repetível**, isto é, a mesma unidade só pode contribuir uma vez para a contagem em causa. Uma morte por pessoa, um lugar por eleição, uma empresa por encerramento satisfazem esta condição. Não a satisfazem acontecimentos repetíveis — consultas, viagens, transações — cujo número pode legitimamente ultrapassar a população em causa. Quando o conjunto se renova durante o período, o limite não tem necessariamente de permanecer fechado: deve ser **aumentado**, isto é, população inicial mais um limite superior de renovação. Esse limite superior é estabelecido a partir dos dados do texto ou de uma verificação externa admissível, nunca de uma estimativa própria do analista: o §5 proíbe substituir o modelo do autor pelo do analista, e uma estimativa aproximada da renovação seria um modelo desse tipo. Sem um limite superior estabelecido, a multiplicação continua a poder ser realizada e é relatada, mas a conclusão do Teste A sobre a impossibilidade **não é decidível por falta de um limite estabelecido**, no sentido do §5. O cálculo e o seu resultado são escritos, a impossibilidade não é afirmada, e esse silêncio não valida o número. Nomeia a unidade contabilizada, o limite adotado e a janela temporal antes de concluir; sem uma unidade não repetível, o simples excesso não estabelece nada.
- **B. Denominador.** O denominador está indicado? É adequado? O denominador é a população, conjunto ou base de referência a que a percentagem se refere, não necessariamente o seu valor numérico absoluto. «9% das vendas mundiais» indica o seu denominador mesmo que o volume total das vendas mundiais não seja fornecido: esse volume pode faltar para converter a percentagem em valor absoluto ou para outro teste de reconstruibilidade, mas a sua ausência não é um defeito de denominador. Uma percentagem sem população ou base de referência identificável não é um dado. Para uma variação ou redução percentual, o período ou valor de comparação deve ser identificável: «+24,5% em termos homólogos» tem uma base de comparação mesmo que os volumes absolutos estejam ausentes; «reduzido em 30%» sem duração, valor ou período de referência não a tem. Uma percentagem cujo denominador ou base de comparação muda ao longo do texto constitui um equívoco.
- **C. Medida de tendência central.** Trata-se de média, mediana ou moda? O texto especifica-o? Qual das três alteraria a conclusão? Uma empresa com vinte e três salários pode mostrar simultaneamente uma média de 6.000, uma mediana de 4.000 e uma moda de 2.000, sem que nenhum dos três números seja falso.
- **D. Taxa de base.** Para toda a afirmação de deteção, rastreio, atribuição ou perfilagem, é fornecida a prevalência na população? Sem ela, uma taxa de deteção de 99% nada diz sobre a probabilidade de um indivíduo assinalado estar realmente afetado. A omissão da prevalência basta para tornar inutilizável uma taxa de deteção e deve ser verificada sistematicamente em textos relativos a segurança, saúde ou fraude.
- **E. Medição, estimativa, modelo, projeção.** O número é medido, estimado a partir de uma amostra, produzido por um modelo ou projetado para o futuro? O texto distingue estas categorias? Um número modelizado apresentado como medição é uma afirmação não suportada, mesmo quando o modelo seja bom.
- **F. Precisão exibida.** O número de algarismos significativos ultrapassa aquilo que o método pode sustentar? Uma projeção ao milhão mais próximo num modelo a dez anos é falsa precisão, e a falsa precisão funciona como marcador de seriedade no sentido de 3.K.
- **G. Reprodutibilidade da contagem.** Para toda a afirmação quantitativa obtida através da seleção ou consulta de um corpus de documentos, de uma cobertura mediática ou de uma base documental, estão indicados a base consultada, a consulta ou critério de seleção utilizado e a unidade de contagem? Se faltar algum destes elementos, a contagem não é reproduzível e não estabelece nada de verificável, qualquer que seja a plausibilidade do resultado. O teste aplica-se à contagem efetuada pelo próprio texto e à contagem que este retransmite quando a apresenta como estabelecida e dela retira a sua própria conclusão. Não se aplica à contagem que o texto atribui explicitamente à sua fonte conservando a respetiva modalidade: no segundo caso, o que é examinado não é a contagem, mas a sua apropriação. **Limite de domínio**: G diz respeito exclusivamente a contagens documentais cujo resultado dependa da escolha da base de dados, da consulta ou do critério de seleção e da unidade de contagem. Não se estende a medições ou séries estatísticas, administrativas, demográficas, científicas ou operacionais correntes pelo simples facto de assentarem numa base de dados; estas pertencem aos Testes A a F e a 3.B conforme o defeito examinado.
...
- um número probatório cuja impossibilidade seja estabelecida pelo Teste A **invalida a proposição quantitativa** que pretendia estabelecer. A sua gravidade para a tese central é determinada em 7.a através do teste de substituição. Como o §2 qualifica agora como probatório um número de que a conclusão poderia prescindir, essa gravidade já não pode ser automática sem contradizer 7.a;
- uma contagem que não seja reproduzível no sentido do Teste G **não estabelece** a subtese empírica que sustenta, sem que por isso fique estabelecida a sua falsidade. Se a contagem for realizada pelo próprio texto, a subtese torna-se uma **afirmação quantitativa não demonstrada e não reproduzível**. Se o texto retransmitir uma contagem externa que apresentara como estabelecida, deixa de poder utilizá-la como resultado demonstrado e deve conservá-la como **afirmação referida**. O efeito sobre a tese central é determinado em 7.a através do teste de substituição e não é automático;
- a palavra **redibitório** é reservada à grelha de 7.a, onde significa que o defeito invalida a tese central. A constatação invalidante da primeira regra não sobe a 7.a sob essa etiqueta, e a segunda regra não estabelece qualquer invalidação, apenas ausência de demonstração: ambas entram em 7.a através do teste de substituição, que é o único que decide o seu grau.
