...
**Wrapper revision:** 0.6.0. This revision targets ARGUS V5.1.0 and adds three analyst-provenance header fields (see Header). The wrapper revision is technical and host-independent; it never changes an ARGUS conclusion.
...
A Case Study is a publication view of an ARGUS analysis.
It does not constitute a new analytical mode and does not modify ARGUS execution or conclusions.
...
- ARGUS V5.1.0
- wrapper revision 0.6.0 (`not applicable` if the execution regime is `context`)
...
- correct ARGUS version and mode declared; wrapper revision 0.6.0 declared when the execution regime is `wrapper`, or marked not applicable when it is `context`; host and execution regime declared; the three analyst-provenance fields (model, version/build, reasoning intensity) recorded verbatim;
