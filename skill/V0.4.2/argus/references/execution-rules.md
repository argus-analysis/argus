# ARGUS — Règles d'exécution du wrapper, révision 0.4.2
...
Toute analyse complète déclare deux numéros distincts : ARGUS V5.0.0 et wrapper 0.4.2.
...
Une analyse destinée à être exportée, archivée ou publiée ne doit pas dépendre uniquement d'un mécanisme de citation propre à l'interface hôte. Les références produites par le wrapper doivent donc exister en texte simple.

### 3.2 Dispositif

Pour chaque contrôle externe abouti ou partiel qui produit une source utilisable :

- placer dans le corps un renvoi `[S1]`, `[S2]`, etc. ;
...
- pour une source en ligne : auteur ou institution, titre, date, lien complet, date de consultation ;
- pour une pièce fournie : titre, date et localisation dans le document — page, section ou paragraphe lorsque disponible ;
- pour un contrôle infructueux : nommer l'objet recherché et déclarer l'absence de résultat, sans fabriquer de référence.

Les renvois `[Sn]` constituent un espace de noms volontairement distinct de notes automatiques éventuelles de l'hôte.
...
3. vérifier que la source citée contient réellement l'information qui lui est attribuée ;
4. rechercher les marqueurs non résolus, crochets vides, placeholders, liens cassés ou renvois sans entrée ;
5. corriger ce qui est observable avant remise ;
6. ne pas déclarer un défaut d'export que l'exécution ne peut pas observer. Si le rendu après export est hors de portée, déclarer seulement cette limite.
